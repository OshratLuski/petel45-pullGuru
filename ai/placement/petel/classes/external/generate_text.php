...
namespace aiplacement_petel\external;

use aiplacement_petel\utils;
...
class generate_text extends external_api {
    ...
    public static function execute(
        ...
        $decodedcustomdata = json_decode($customdata, true);
        ...
        if (!utils::is_petel_placement_action_available($context, $Reflectionclass->getShortName(), \core_ai\aiactions\generate_text::class)) {
            throw new \moodle_exception('notavailable', 'aiplacement_petel');
        }
        ...
        $logdata = (object) [
            'contextid' => $contextid,
            'instanceid' => $decodedcustomdata['instanceid'],
            'action' => $Reflectionclass->getShortName(),
            'type' => $type,
            'request' => $prompttext,
            'response' => $responsedata['generatedcontent'],
            'customdata' => $customdata,
        ...
        $petel_log = new \aiplacement_petel\persistent\log(0, $logdata);
        $petel_log->create();

        if ($response->get_errorcode() !== 0) {
            throw new \moodle_exception('aiplacementerror', 'aiplacement_petel', null, $response->get_errormessage());
        }
        // Return the response.
        return [
            'success' => $response->get_success(),
