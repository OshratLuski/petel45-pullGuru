...
namespace aiprovider_openai_adv;
...
class provider extends \core_ai\provider {
    /** @var string The openai_adv API key. */
    ...
    public function __construct() {
        ...
        $this->apikey = get_config('aiprovider_openai_adv', 'apikey');
        // Get api org id from config.
        $this->orgid = get_config('aiprovider_openai_adv', 'orgid');
        // Get global rate limit from config.
        $this->enableglobalratelimit = get_config('aiprovider_openai_adv', 'enableglobalratelimit');
        $this->globalratelimit = get_config('aiprovider_openai_adv', 'globalratelimit');
        // Get user rate limit from config.
        $this->enableuserratelimit = get_config('aiprovider_openai_adv', 'enableuserratelimit');
        $this->userratelimit = get_config('aiprovider_openai_adv', 'userratelimit');
    }
     ...
     * but don't pass any personal data to openai_adv.
     *
    ...
    public function add_authentication_headers(RequestInterface $request): RequestInterface {
        return $request
            ...
            ->withAddedHeader('openai_adv-Organization', $this->orgid);
    }
    ...
    public function get_action_settings(
        ...
        if ($actionname === 'generate_text' || $actionname === 'summarise_text') {
            ...
            $settings[] = new \admin_setting_configtext(
                "aiprovider_openai_adv/action_{$actionname}_model",
                new \lang_string("action:{$actionname}:model", 'aiprovider_openai_adv'),
                new \lang_string("action:{$actionname}:model_desc", 'aiprovider_openai_adv'),
                'gpt-4o',
                PARAM_TEXT,
            );
            // Add API endpoint.
            $settings[] = new \admin_setting_configtext(
                "aiprovider_openai_adv/action_{$actionname}_endpoint",
                new \lang_string("action:{$actionname}:endpoint", 'aiprovider_openai_adv'),
                '',
                'https://api.openai_adv.com/v1/chat/completions',
                PARAM_TEXT,
            );
            // Add system instruction settings.
            $settings[] = new \admin_setting_configtextarea(
                "aiprovider_openai_adv/action_{$actionname}_systeminstruction",
                new \lang_string("action:{$actionname}:systeminstruction", 'aiprovider_openai_adv'),
                new \lang_string("action:{$actionname}:systeminstruction_desc", 'aiprovider_openai_adv'),
            ...
            $settings[] = new \admin_setting_configtext(
                ...
                new \lang_string("action:{$actionname}:endpoint", 'aiprovider_openai_adv'),
                '',
                'https://api.openai_adv.com/v1/images/generations',
                PARAM_TEXT,
            );
        }
