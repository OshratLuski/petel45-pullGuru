...
/**
 * Base trait for message petel tests.
 *
...
trait message_petel_test_helper {

    /**
     * Send a fake unread petel notification.
     *
     ...
     */
    protected function send_fake_unread_petel_notification(\stdClass $userfrom, \stdClass $userto,
        ...
        $petel = new stdClass();
        $petel->notificationid = $id;

        $DB->insert_record('message_petel_notifications', $petel);

        return $id;
    }

    /**
     * Send a fake read petel notification.
     *
     * {@link message_send()} does not support transaction, this function will simulate a message
     * sent from a user to another. We should stop using it once {@link message_send()} will support
     * transactions. This is not clean at all, this is just used to add rows to the table.
     ...
     * @return int the id of the message
     */
    protected function send_fake_read_petel_notification(\stdClass $userfrom, \stdClass $userto, string $message = 'Hello world!',
