...
class restore_create_categories_and_questions extends restore_structure_step {
    ...
    protected function define_structure() {
        ...
        if ($before40) {
            ...
            $paths [] = new restore_path_element('competency', '/question_categories/question_category/question_bank_entries/'.
                    'question_bank_entry/question_version/question_versions/questions/question/competencies/competency');
        }
    ...
    protected function process_localmetadata($data) {
        global $DB;
        $data = (object) $data;

        $oldquestion = $this->get_old_parentid('question');
        $sql = "SELECT lm.*
                FROM {local_metadata} AS lm
                LEFT JOIN {local_metadata_field} AS lmf ON lmf.id=lm.fieldid
                WHERE lmf.contextlevel = 9100 AND lm.instanceid = ?";
        if ($rows = $DB->get_records_sql($sql, [$oldquestion])) {
            foreach ($rows as $row) {
                unset ($row->id);
                $row->instanceid = $this->get_new_parentid('question');
                if ($dataid =
                        $DB->get_field('local_metadata', 'id', ['instanceid' => $row->instanceid, 'fieldid' => $row->fieldid])) {
                    $row->id = $dataid;
                    $DB->update_record('local_metadata', $row);
                } else {
                    $DB->insert_record('local_metadata', $row);
                }
            }
        }
    }
