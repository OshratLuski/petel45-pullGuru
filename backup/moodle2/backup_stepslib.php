...
class backup_questions_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        // BACKUP data from PLUGIN local_metadata.
        $localmetadatas = new backup_nested_element('localmetadatas');
        $localmetadata = new backup_nested_element('localmetadata', array('id'),
                array('instanceid', 'fieldid', 'data', 'dataformat'));

        $question->add_child($localmetadatas);
        $localmetadatas->add_child($localmetadata);

        $localmetadata->set_source_sql("SELECT lm.*
                                            FROM {local_metadata} AS lm
                                            LEFT JOIN {local_metadata_field} AS lmf ON lmf.id=lm.fieldid
                                            WHERE lmf.contextlevel = 9100 AND lm.instanceid = ?", [backup::VAR_PARENTID]);
