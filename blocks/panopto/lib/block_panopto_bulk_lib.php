...
function panopto_bulk_sanitize_and_get_mappings($params, $skipimports) {
    ...
    if (isset($params[0])) {
        $oldpanoptocourses = $DB->get_records('block_panopto_foldermap', ['moodleid' => $params[0]], null, 'id,moodleid');
    } else {
        $oldpanoptocourses = $DB->get_records('block_panopto_foldermap', null, null, 'id,moodleid');
    }
