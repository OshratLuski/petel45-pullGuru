...
class controlmenu extends \core_courseformat\output\local\content\section\controlmenu {
    ...
    public function section_control_items() {
        ...
        if (has_capability('moodle/course:update', $coursecontext)) {
            $controls['copylinksection'] = [
                    'url' => 'javascript::void(0);',
                    'icon' => 't/copy',
                    'name' => get_string('copylinksection', 'community_sharewith'),
                    'pixattr' => ['class' => ''],
                    'attr' => [
                            'class' => '',
                            'data-handler' => 'copyLinkSection',
                            'data-sectionid' => $section->id,
        ...
        if ($section->section == $this->format->get_viewed_section()) {
            ...
            // Delete section for single view.
            if (course_can_delete_section($course, $section)) {
                ...
                $params = [
                        'id' => $this->format->get_courseid(),
                        'deletesection' => $section->sectionnum,
                        'confirm' => 1,
                        'sesskey' => sesskey(),
                ];
                $url = new moodle_url(
