...
class section extends \core_courseformat\output\local\content\section {
    ...
    public function export_for_template(\renderer_base $output, bool $ajax = false): stdClass {
        ...
        // If not presents activities.
        if (!$DB->count_records('course_modules', ['course' => $course->id, 'section' => $section->id])) {
            $lazyload = false;
            $cmload = true;
        }
        ...
        if ((isset($course->showsection0title) && !$course->showsection0title && $this->section->section === 0) ||
    ...
    protected function add_editor_data(stdClass &$data, renderer_base $output): bool {
        ...
        // Enable section edit && $this->format->get_sectionid() != $this->section->id
