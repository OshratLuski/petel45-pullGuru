...
function format_flexsections_cm_grade_status(cm_info $mod) {
    ...
    if (format_flexsections_has_teacher_capability($mod->id)) {
        if (in_array($mod->modname, ['assign', 'quiz', 'questionnaire', 'hvp', 'vpl'])) {
            $tooltip       = '';
            ...
            switch ($mod->modname) {
                ...
                case 'vpl':
                    ...
                    $query = "
                        ...
                        INNER JOIN {vpl_submissions} vs ON (vs.vpl = gi.iteminstance AND vs.userid = gg.userid)                        
                        WHERE gi.itemmodule='vpl' AND gi.iteminstance=? AND gg.`finalgrade` IS NULL
                        GROUP BY gg.userid
                    ";

                    $data = $DB->get_records_sql($query, [$mod->instance]);
                    $countwithoutgrades    = count($data);
                    ...
                    $query = "
                        ...
                        FROM {grade_items} gi
                        LEFT JOIN {grade_grades} gg ON (gi.id = gg.itemid)
                        WHERE gi.itemmodule='vpl' AND gi.iteminstance=? AND gg.`finalgrade` IS NOT NULL                        
                    ";

                    $data = $DB->get_records_sql($query, [$mod->instance]);
                    $havegrade    = count($data);

                    $notsubmitted = $countmaxusers - $havegrade - $countwithoutgrades;
                    ...
                    $url = new moodle_url('/mod/vpl/grade.php', ['id' => $mod->context->instanceid]);

                    // Tooltip.
                    if ($countwithoutgrades) {
                        $tooltip .= '<div>' . $countwithoutgrades . ' ' . get_string('vplwithoutgrades', 'format_flexsections') . ' </div>';
                    }

                    // Status Y תלמידים טרם הגישו.
                    if ($notsubmitted) {
                        $tooltip .= '<div>' . $notsubmitted . ' ' . get_string('vplnotsubmitted', 'format_flexsections') . ' </div>';
                    }

                    // Status V ניתן ציון (אוטומטי או ידני).
                    if ($havegrade) {
                        $tooltip .= '<div>' . $havegrade . ' ' . get_string('vplhavegrade', 'format_flexsections') . ' </div>';
                    }
...
function format_flexsections_cm_submission_data(cm_info $mod, $userid = 0) {
    ...
    if (!in_array($mod->modname, ['quiz', 'assign', 'questionnaire', 'hvp', 'vpl'])) {
        return false;
    }
    ...
    switch ($mod->modname) {
        ...
        case 'vpl':
            $tmod->duedate      = $extra->startdate;
            $tmod->cutoffdate   = $extra->duedate;
            $tmod->requiregrade = true;
            ...
            $query = "
                ...
                WHERE gi.itemmodule='vpl' AND gi.iteminstance=?
                ";

            $rowas = $DB->get_record_sql($query, [$userid, $mod->instance]);
            if ($rowas && !empty($rowas->finalgrade)) {
                $tmod->submitted = true;
                $tmod->grade     = $rowas->finalgrade;
                $tmod->viewgrade = 1;
            }
...
function format_flexsections_cm_submission_status(cm_info $mod) {
    ...
    if (format_flexsections_has_teacher_capability($mod->id)) {
         // Teacher view.
         if ($tmod->cutoffdate) {
            $seconds_since_due = time() - $tmod->cutoffdate;
            $days_since_due = floor($seconds_since_due / 86400);
    
            if ($tmod->cutoffdate > time() || $days_since_due <= 7) {
                // Show due date if it's in the future or within the last 7 days.
                $res->modstatus = date("d/m/Y H:i", $tmod->cutoffdate);
            } else {
                // More than 7 days have passed – don't show anything.
                $res->modstatus = '';
            }
        } else {
            // No cutoff date defined.
        ...
        // Not submitted yet, and due date is in the future.
        ...
        // Due date has passed, but show "overdue" only if it's within the last 7 days.
        if (!$tmod->submitted && $tmod->cutoffdate && $tmod->cutoffdate <= time()) {
            $seconds_since_due = time() - $tmod->cutoffdate;
            $days_since_due = floor($seconds_since_due / 86400);

            if (true) { // $days_since_due <= 7
