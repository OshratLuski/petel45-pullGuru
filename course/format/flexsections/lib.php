...
class format_flexsections extends core_courseformat\base {
    ...
    public function get_course_index_display(): int {
        return (int) isset($this->get_course()->courseindexdisplay) ? $this->get_course()->courseindexdisplay : 0;
    }
    ...
    public function get_accordion_setting(): bool {
        return (bool)isset($this->get_course()->accordion) && $this->get_course()->accordion;
    }
     ...
     */
    public function course_format_options_old($foreditform = false) {
    ...
    public function can_move_section_to($section, $parent, $before = null) {
        ...
        if ($before) {
            ...
            if (!$before || $before->parent !== $parent->section) {
                $before = $this->get_first_child($parent->section);
            }
        }
    ...
    public function update_section_format_options($data) {
        ...
        if (!array_key_exists('image', array($data))) {
            return $changes;
        }
    ...
    /**
     * Get the first child section of a parent section.
     *
     * This function iterates through all sections and returns the first section
     * that matches the specified parent section ID.
     *
     * @param int $parentSectionId The ID of the parent section.
     * @return \section_info|null The first child section, or null if none found.
     */
    protected function get_first_child($parentSectionId) {
        foreach ($this->get_sections() as $section) {
            if ($section->parent == $parentSectionId) {
                return $section;
            }
        }
        return null;
...
function format_flexsections_add_back_link_to_cm(): ?cm_info {
    ...
    if ($PAGE->course
            ...
            && isset(course_get_format($PAGE->course)->get_course()->cmbacklink) ? course_get_format($PAGE->course)->get_course()->cmbacklink : true
            && $PAGE->pagelayout === 'incourse' // Only view pages with the incourse layout (not popup, embedded, etc).
            && isset($PAGE->cm->sectionnum) // Do not display in activities in General section.
