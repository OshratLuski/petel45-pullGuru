...
class format_flexsections_external extends external_api {
    ...
    public static function version_download_parameters() {
        return new external_function_parameters(
            array(
    ...
    public static function version_download($cmid, $courseid) {
        global $USER, $CFG;

        // Validate parameters.
        $params = self::validate_parameters(self::version_download_parameters(), array(
            'cmid' => $cmid,
            'courseid' => $courseid,
        ));
        ...
        if (class_exists('\community_sharewith\funcs')) {
            $metadata = [
                    "notification" => 'bankdownload',
                    'referer' => 'oercatalog',
                    'activitysequence' => 0,
                    "callbackfunc" => '\community_sharewith\funcs::change_activity_name'
            ];

            $metadata = json_encode($metadata);

            \community_sharewith\funcs::add_task('activitycopy', $USER->id, $USER->id, null, $courseid, null,
                    $cm->section, null, $mid, $metadata);
        }

        $message = get_string('downloadtheversionomessage', 'format_flexsections');

        return $message;
    }

    public static function version_download_returns() {
    ...
    public static function get_info($cmid, $courseid) {
        $params = self::validate_parameters(self::get_info_parameters(), array(
            'cmid' => $cmid,
            'courseid' => $courseid,
        ));
        ...
        $return = (object)[
            'title' => get_string('titlepopupinfoblock', 'format_flexsections'),
            'body' => '',
        ];
        ...
        $a->version = $version;
        $a->oerversion = intval(trim($data->metadata_version));
        $a->description = $version;
        $return->body = get_string('textversion', 'format_flexsections', $a);
    ...
    public static function get_info_returns() {
        return new external_single_structure(
            array(
                'title' => new external_value(PARAM_TEXT, 'activitiinfotitle'),
                'body' => new external_value(PARAM_RAW, 'activityinfotext'),
            )
        );
    }
