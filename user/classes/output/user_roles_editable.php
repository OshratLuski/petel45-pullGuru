...
class user_roles_editable extends \core\output\inplace_editable {
    ...
    public static function update($itemid, $newvalue) {
        ...
        $teachercohortid = $DB->get_field('cohort', 'id', ['idnumber' => $CFG->siteteachers]);

        if (in_array($teacherroleid, $roleids) && !cohort_is_member($teachercohortid, $userid)) {
            $userfullname = $DB->get_field('user', 'firstname', ['id' => $userid]) . ' ' . $DB->get_field('user', 'lastname', ['id' => $userid]);
            $a = (object) ['username' => $userfullname];
            throw new \moodle_exception(get_string('no_teacher_permission', 'local_petel', $a));
        }
