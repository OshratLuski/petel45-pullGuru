...
class auth extends \auth_plugin_base {
    ...
    public function error_page_moe_user_notfound($user) {
        ...
        if ($error == 'invalidkey') {
            $user->error = get_string('error_invalidkey', 'auth_saml2');
        }
        echo $OUTPUT->header();
    ...
    public function error_page_moe_update_idnumber($user){
        global $PAGE, $OUTPUT, $SESSION, $DB, $CFG;
        unset($SESSION->wantsurl);

        $idnumber = optional_param('idnumber', null, PARAM_TEXT);
        $action = optional_param('action', null, PARAM_TEXT);
        if ($action == 'updateidnumber') {
            //if user exist
            if (!empty($user->id)) {
                if (empty($idnumber) || !$this->validate_israeli_idnumber($idnumber) || $DB->count_records('user', ['idnumber' => $idnumber])) {
                    $user->error = get_string('error_validation_idnumber', 'auth_saml2');
                }
                if (empty($user->error)) {
                    require_once($CFG->dirroot . '/user/lib.php');
                    $user->idnumber = $idnumber;
                    user_update_user($user, false);
                    redirect($CFG->wwwroot . '/auth/saml2/login.php');
                }
            } else {
                if (empty($idnumber) || !$this->validate_israeli_idnumber($idnumber)) {
                    $user->error = get_string('error_validation_idnumber', 'auth_saml2');
                }
                if ($tmpuser = $DB->get_record('user', ['idnumber' => $idnumber])) {
                    require_once($CFG->dirroot . '/user/profile/lib.php');
                    $customfields = profile_user_record($tmpuser->id);
                    if (!empty($customfields->moe_exidentifier)) {
                        $user->error = get_string('error_validation_idnumber', 'auth_saml2');
                    } else {
                        $tmpuser->profile_field_moe_exidentifier = $user->profile_field_moe_exidentifier;
                        profile_save_data($tmpuser);
                        require_once($CFG->dirroot . '/lib/moodlelib.php');

                        $from = \core_user::get_support_user();
                        $subject = get_string('warning_send_email_subject', 'auth_saml2');
                        $message = get_string('warning_send_email_message', 'auth_saml2', $idnumber);
                        $email_sent = email_to_user($tmpuser, $from, $subject, strip_tags($message), $message);
                        redirect($CFG->wwwroot . '/auth/saml2/login.php');
                    }
                }
                if (empty($user->error)) {
                    $SESSION->auth2_saml_idnumber = $idnumber;
                    redirect($CFG->wwwroot . '/auth/saml2/login.php');
                }
            }
        }
        ...
        $PAGE->set_heading(get_string('error_page_moe_user_title', 'auth_saml2'));
        echo $OUTPUT->header();
        echo $OUTPUT->render_from_template('auth_saml2/error_page_moe_update_idnumber', $user);
        echo $OUTPUT->footer();
        exit();
    }
    ...
    public function saml_login_complete($attributes) {
        ...
        // Get parameter form http://schemas.education.gov.il/ws/2015/01/identity/claims/zehut if exists.
        $idnumber = isset($attributes['zehut'][0]) ? $attributes['zehut'][0] : null;
        ...
        // User is not exist check by idnumber.
        if (!$user && !in_array($CFG->instancename, ['feinberg', 'tutorials']) && empty($SESSION->auth2_saml_idnumber) && empty($idnumber)) {
            $user = new stdClass();
            $user->username = $uid;
            $user->profile_field_moe_exidentifier = $uid;
            $this->error_page_moe_update_idnumber($user);
        }

        // If user exist but no idnumber.
        if ($user && empty($user->idnumber) && empty($idnumber)) {
            $this->error_page_moe_update_idnumber($user);
        }
        ...
        if (!$user) {
            ...
            if ($this->config->autocreate) {
                ...
                if (isset($CFG->instancename) && in_array($CFG->instancename, ['feinberg', 'tutorials'])) {
                    ...
                    if (empty($user->idnumber)) {
                        $user->idnumber = !empty($idnumber) ? $idnumber : $SESSION->auth2_saml_idnumber;
                    }
            ...
            if (!empty($SESSION->auth2_saml_idnumber)){
                $user->idnumber = $SESSION->auth2_saml_idnumber;
            }
    ...
    protected function check_whitelisted_ip_redirect() {
        foreach ($this->metadataentities as $idpentity) {
            if ($idpentity->whitelist && \core\ip_utils::is_ip_in_subnet_list(getremoteaddr(), $idpentity->whitelist ?? '')) {
