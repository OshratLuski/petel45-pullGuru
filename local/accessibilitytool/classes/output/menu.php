...
class menu implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // Reset.
        $reseturl = new \moodle_url('/local/accessibilitytool/manage.php', ['reset' => 1, 'atr' => $this->data->atr]);
        $this->data->reseturl = $reseturl->out();
    ...
    public function set_default_preference() {
        $types = ['contrast', 'stripstyles', 'bold', 'spacing', 'readtome', 'gridformat', 'font', 'size'];
        foreach ($types as $type) {
            unset_user_preference("accessibilitytool_" . $type);
        }
    }
}
