 ...
 * Menu profile field.
 *
 * @package    profilefield_multimenu
 * @copyright  2007 onwards Shane Elliot {@link http://pukunui.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace metadatafieldtype_multimenu;
...
class metadata extends \local_metadata\fieldtype\metadata {
    ...
    public function __construct($fieldid = 0, $instanceid = 0) {
        ...
        foreach ($options as $key => $option) {
            $this->options[trim($option)] = format_string(trim($option)); // Multilang formatting with filters.
        }
        ...
        if ($this->data !== null) {
            ...
            //if (in_array($key, array_values($this->options))) {
                $this->data = $key;
                $this->datakey = $key;
            //}
        }
    ...
    public function edit_field_add($mform) {
        ...
        $select = $mform->addElement('select', $this->inputname, format_string($this->field->name), $this->options, $attr);
