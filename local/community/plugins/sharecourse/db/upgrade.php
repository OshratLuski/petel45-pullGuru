...
/**
 * Local plugin "sharecourse" - Upgrade plugin tasks
 *
 ...
 * @return bool result
 */
function xmldb_community_sharecourse_upgrade($oldversion) {

    global $DB;
    $dbman = $DB->get_manager();

    if ($oldversion < 2020110822) {

        if ($obj = $DB->get_record('local_metadata_field', ['shortname' => 'cshared_at'])) {
            $obj->name = 'תאריך שיתוף';

            $DB->update_record('local_metadata_field', $obj);
        }
    }
