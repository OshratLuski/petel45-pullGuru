...
abstract class local_reminder_activity_handler {
    ...
    /**
     * Checks whether a submission exists for a specific user in a given assignment module.
     * This function is used to determine if the user has submitted an assignment,
     * regardless of their completion status in the completion system.
     *
     * @param object $coursemodule The course module object (must belong to mod_assign).
     * @param int $userid The user ID to check submission for.
     * @return bool True if a submission with status 'submitted' exists for the user; false otherwise.
     */
    protected function assignment_submission_exists($coursemodule, $userid): bool {
        global $DB;

        // Get the assignment instance ID from the course module.
        $assignid = $DB->get_field('assign', 'id', ['id' => $coursemodule->instance]);

        // Check if the user has a submitted record in mdl_assign_submission.
    ...
    protected function check_completion_status($course, $coursemodule, $userid) {
        ...
        if ($completion->is_enabled($coursemodule)) {
            // Special case for mod_assign: use submission status if exists.
            if ($coursemodule->modname === 'assign' && $this->assignment_submission_exists($coursemodule, $userid)) {
