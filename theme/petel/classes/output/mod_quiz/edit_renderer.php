...
namespace theme_petel\output\mod_quiz;
...
class edit_renderer extends \mod_quiz\output\edit_renderer {
    ...
    public function edit_page(quiz_settings $quizobj, structure $structure,
        ...
        // Disable button.
        $disablebutton = false;
        $disablebuttonshare = false;
        $hasattempts = false;
        if ($DB->record_exists('quiz_attempts', array('quiz' => $quizobj->get_quizid(), 'preview' => 0))) {
            $disablebutton = true;
            $hasattempts = true;
        }
        ...
        $buttondeleteoptions = array(
                ...
                'value' => get_string('deleteselectedcustom', 'mod_quiz'),
                'class' => 'btn btn-secondary ml-2 btn-delete-questions',
                'data-action' => 'toggle',
                'data-togglegroup' => $this->togglegroup,
                'data-toggle' => 'action',
        );
        if($disablebutton) $buttondeleteoptions['disabled'] = 'disabled';
        if($hasattempts) $buttondeleteoptions['data-hasattempts'] = '1';
        ...
        $buttonshare = array(
                'type' => 'button',
                'value' => get_string('sharingbutton', 'community_sharequestion'),
                'class' => 'btn btn-secondary ml-3 btn-share-questions',
                'data-togglegroup' => $this->togglegroup,
                'data-toggle' => 'action',
                'data-action' => 'sharequestion'
        );
        if($disablebuttonshare) $buttonshare['disabled'] = 'disabled';
        if($hasattempts) $buttonshare['data-hasattempts'] = '1';

        $sharebutton = '';
        if (has_capability('community/sharequestion:questioncopy', \context_course::instance($COURSE->id), $USER->id)) {
            $sharebutton = html_writer::tag('button', get_string('sharingbutton', 'community_sharequestion'), $buttonshare) .
            " " ;
        }

        $output .= html_writer::tag('div',
                html_writer::tag('button', get_string('deleteselectedcustom', 'mod_quiz'), $buttondeleteoptions) .
                " " .
                $sharebutton.
     ...
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl, $buttondisabled = false) {
        ...
        if (!$structure->can_be_repaginated() || $buttondisabled) {
    ...
    public function page_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $checkboxall = '<span><input class="checkbox-select-all-questions mr-3 ml-4" type="checkbox"></span>';
        if ($structure->is_first_slot_on_page($slot)) {
            ...
            $output .= html_writer::tag('li', $checkboxall . $page . $addmenu . $addquestionform,
                    array('class' => 'pagenumber activity yui3-dd-drop page', 'id' => 'page-' . $pagenumber,
                            'style' => 'padding-bottom: 21px; padding-right: 10px;'));
        }
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $trigger = html_writer::tag('span', get_string('add', 'quiz'), array('class' => 'add-menu btn btn-secondary btn-sm'));
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank', 'addquestionfrommycourses', 'addquestionfromoer'), 'quiz');
        }
        ...
        // Add questions from my courses.
        $params = array('cmid' => $structure->get_cmid());
        $actions['addquestionfrommycourses'] = new \action_menu_link_secondary(
                new \moodle_url($pageurl, $params),
                new \pix_icon('t/add', $str->addquestionfrommycourses, 'moodle', array('class' => 'iconsmall', 'title' => '')),
                $str->addquestionfrommycourses, array('class' => 'cm-edit-action addquestionfrommycourses',
                        'data-action' => 'addquestionfrommycourses', 'data-cmid' => $structure->get_cmid())
        );

        // Add questions from oer catalog.
        $params = array('cmid' => $structure->get_cmid());
        $actions['addquestionfromoer'] = new \action_menu_link_secondary(
                new \moodle_url($pageurl, $params),
                new \pix_icon('t/add', $str->addquestionfromoer, 'moodle', array('class' => 'iconsmall', 'title' => '')),
                $str->addquestionfromoer, array('class' => 'cm-edit-action addquestionfromoer',
                        'data-action' => 'addquestionfromoer', 'data-cmid' => $structure->get_cmid())
        );
    ...
    public function question(structure $structure, int $slot, \moodle_url $pageurl) {
        ...
        $output .= $this->render_from_template('theme_petel/mod_quiz/question_slot', $data);
     ...
     * @return string HTML to output.
     */
    public function question_number(string $editablenumber, string $uncustomisednumber = null) {
        if (is_numeric($editablenumber)) {
            $editablenumber = html_writer::span(get_string('question'), 'accesshide') . ' ' . $editablenumber;
        }
        return html_writer::tag('span', $editablenumber, array('class' => 'slotnumber'));
    }
     ...
     * @return string HTML to output.
     */
    public function question_metadata_icon(structure $structure, $slot, $pageurl) {
        global $COURSE;

        $qid = 0;
        $qpage = 0;

        if(get_config('metadatacontext_question', 'metadataenabled') != 1) return '';
        if (!is_siteadmin() && !can_edit_in_category($COURSE->category)) return '';
        ...
        $image = $this->pix_icon('i/db', $strmetadata);

        return $this->action_link($url, $image, null, array('title' => $strmetadata,
            'class' => 'cm-edit-action editing_duplicate', 'data-action' => 'duplicate'));
    }
     ...
     * @return string HTML to output.
     */
    public function question_review_icon(structure $structure, $slot, $pageurl) {
        global $COURSE, $DB;
        ...
        $oerqid = \local_metadata\mcontext::question()->get($qid, 'qidhistory');

        $question = new \community_oer\question_oer;
        $data = $question->query()->compare('qid', $oerqid)->get();
        $data = array_values($data);
        if(isset($data[0])){
            $strmetadata = get_string('comments_box', 'community_oer');

            // Count reviews to question.
            $reviews = $DB->get_records('community_oer_reviews', array('objid' => $oerqid, 'reviewtype' => 'question'));

            $html = '
                <div class="text-primary mr-2" data-objid="'.$oerqid.'" data-type="question" data-handler="showReview" title="'.$strmetadata.'">
                    <i class="fal fa-comment"></i>
                    <span class="text-body m-0">'.count($reviews).'</span>
                </div>
            ';
            return $html;
        }else{
