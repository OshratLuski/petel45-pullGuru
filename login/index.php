...
function verify_recaptcha_v2($recaptcha_response, $recaptcha_url, $secret_key){
    $response = file_get_contents($recaptcha_url . "?secret=" . $secret_key . "&response=" . $recaptcha_response . "&remoteip=" . $_SERVER['REMOTE_ADDR']);
    return json_decode($response);
}

function verify_recaptcha_v3($recaptcha_response, $recaptcha_url, $secret_key){
    $recaptcha_url = $recaptcha_url . '/siteverify';
    $response = file_get_contents($recaptcha_url . '?secret=' . $secret_key . '&response=' . $recaptcha_response);
    return json_decode($response, true);
}

function trigger_recaptcha_failed_event($username, $password){
    ...
    $event = \core\event\recaptcha_failed::create([
            'context' => context_system::instance(),
            'other' => [
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if (!isset($SESSION->failedloginattempts)) {
    ...
    // Verify ReCaptcha V2.
    if (isset($CFG->recaptchav2enable) && $CFG->recaptchav2enable && isset($CFG->recaptchav2url) && isset($CFG->recaptchav2privatekey) &&
            $SESSION->failedloginattempts >= $CFG->failedlogins_without_recaptcha) {
        if (!empty($myform['g-recaptcha-response'])) {
            $response = verify_recaptcha_v2($myform['g-recaptcha-response'], $CFG->recaptchav2url, $CFG->recaptchav2privatekey);
            if (!isset($response->success) || !$response->success) {
                $errormsg = get_string('configrecaptchav2failed', 'local_petel');
            }
        } else {
            $errormsg = get_string('configrecaptchav2failed', 'local_petel');
            trigger_recaptcha_failed_event($myform['username'], $myform['password']);
        }
    }
    ...
    if (isset($CFG->recaptchav3enable) && $CFG->recaptchav3enable && isset($CFG->recaptchav3url) && isset($CFG->recaptchav3privatekey) &&
            $SESSION->failedloginattempts >= $CFG->failedlogins_without_recaptcha) {
        if (!empty($myform['recaptcha_response'])) {
            $responseKeys = verify_recaptcha_v3($myform['recaptcha_response'], $CFG->recaptchav3url, $CFG->recaptchav3privatekey);
            ...
            if (!$responseKeys["success"] || $responseKeys["score"] < $recaptchascore) {
                $errormsg = get_string('configrecaptchav3failed', 'local_petel');
            }
        } else {
...
if (!empty($SESSION->loginerrormsg) || !empty($SESSION->logininfomsg)) {
    ...
    if (isset($errormsg) && !empty($errormsg) && isset($SESSION->failedloginattempts)) {
...
// Include the reCAPTCHA script only if the reCAPTCHA is set
if (isset($CFG->recaptchav3enable) && $CFG->recaptchav3enable && isset($CFG->recaptchav3sitekey) && isset($CFG->recaptchav3url) &&
        $SESSION->failedloginattempts >= $CFG->failedlogins_without_recaptcha) {
    $recaptchaSiteKey = $CFG->recaptchav3sitekey;
    $recaptchaUrl = $CFG->recaptchav3url;
