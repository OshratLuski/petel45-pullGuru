...
$eventparam = array(
        ...
        'other' => array(
                ...
                'phone' => optional_param('phone', '', PARAM_ALPHANUMEXT),
        ),
        'relateduserid' => 1
);
...
if (!empty($eventparam['other']['email'])){
    $SESSION->password_reset_email = $eventparam['other']['email'];
    $SESSION->password_reset_phone = '';
}
elseif (!empty($eventparam['other']['phone'])){
    $SESSION->password_reset_email = '';
    $SESSION->password_reset_phone = $eventparam['other']['phone'];
}
else{
    $SESSION->password_reset_email = $eventparam['other']['email'];
    $SESSION->password_reset_phone = $eventparam['other']['phone'];
