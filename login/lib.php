...
function core_login_process_password_reset($username, $email, $phone) {
          ...
          or !has_capability('moodle/user:changeownpassword', $systemcontext, $user->id)) {
            ...
            if (!empty($sendemail) && $CFG->issmsvalidation) {
                ...
                if ($sendresult) {
                    ...
                    throw new \moodle_exception('cannotsmsconfirm');
                }
            }
    ...
    if ( !empty($CFG->protectusernames && $CFG->issmsvalidation)) {
...
function core_login_process_password_set($token) {
    ...
    //Reset password based on username.
    if (empty($SESSION->password_reset_email) && empty($SESSION->password_reset_phone)) {
        $SESSION->password_reset_email = $user->email;
        $SESSION->password_reset_phone = $user->phone2;
    }
    if (empty($user) || ($user->timerequested < (time() - $pwresettime - DAYSECS)) || ($user->phone2 != $SESSION->password_reset_phone && $user->email != $SESSION->password_reset_email)) {
...
function core_login_generate_password_reset ($user) {
    ...
    if($CFG->issmsvalidation){
        ...
        $smscode = rand(10,99).rand(10,99).rand(10,99);
