...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        initializer: function(options) {
            ...
            this.filemanager.all('.fp-vb-icons,.fp-vb-tree,.fp-vb-details').setAttribute('aria-pressed', 'false');
            this.filemanager.all(viewmodeselectors[this.viewmode]).addClass('checked');
            this.filemanager.all(viewmodeselectors[this.viewmode]).setAttribute('aria-pressed', 'true');
        ...
        setup_buttons: function() {
            ...
            this.filemanager.all('.fp-vb-icons,.fp-vb-tree,.fp-vb-details').
                on('click', function(e) {
                    ...
                    var self = this;
                    if (!self.is_disabled() && (!viewbar || !viewbar.hasClass('disabled'))) {
                        require(['core/str'], function(Str) {
                            Str.get_string('currentview', 'local_petel')
                                .then(function(string) {
                                    self.filemanager.all('.fp-vb-icons,.fp-vb-tree,.fp-vb-details').removeClass('checked')
                                    self.filemanager.all('.fp-vb-icons,.fp-vb-tree,.fp-vb-details').setAttribute('aria-pressed', 'false');
                                    ...
                                    e.currentTarget.setAttribute('title', string + ' ' + e.currentTarget.getAttribute('title'));
                                    self.render();
                                    self.filemanager.one('.fp-content').setAttribute('tabIndex', '0');
                                    self.filemanager.one('.fp-content').focus();
                                    self.set_preference('recentviewmode', self.viewmode);
                                })
                        });
                    }
                }, this);
        ...
        print_path: function() {
            ...
            if (p && p.length!=0 && this.viewmode != 2) {
                require(['core/str'], function(Str) {
                    Str.get_string('currentfolder', 'local_petel')
                        .then(function(string) {
                                for(var i = 0; i < p.length; i++) {
                                    var el = self.pathnode.cloneNode(true);
                                    self.pathbar.appendChild(el);
                                    ...
                                    el.one('.fp-path-folder-name')
                                        .setAttribute('aria-label', string + Y.Escape.html(p[i].name))
                                        .setContent(Y.Escape.html(p[i].name)).
                                ...
                                self.pathbar.removeClass('empty');
                        })
                });
            }
        },
