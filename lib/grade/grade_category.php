...
class grade_category extends grade_object {
     ...
     * @return array
     */
    public static function fetch_course_tree($courseid, $include_category_items=false, $autosort = false) {
        $course_category = grade_category::fetch_course_category($courseid);
        $category_array = array('object'=>$course_category, 'type'=>'category', 'depth'=>1,
                                'children'=>$course_category->get_children($include_category_items, $autosort));
     ...
     */
    public function get_children($include_category_items=false, $autosort=false) {
        global $DB;
        ...
        // Auto-sort.
        if ($autosort) {
            $items = $this->autosort_items_in_course();
        }
    ...
    public function autosort_items_in_course() {
        global $DB;

        if ($items = $DB->get_records('grade_items', array('courseid' => $this->courseid))) {
            ...
            foreach ($items as $ik => $item) {
                if ($item->itemtype == 'mod') {
                    $sortkeys[]                      = $item->sortorder;
                    $iteminstances[$item->sortorder] = [$item->iteminstance, $item->itemmodule, $ik];
                    if (!in_array($item->itemmodule, $modulenames, true)) {
                        array_push($modulenames, $item->itemmodule);
                    }
                }
            }
            $sql = "SELECT cs.id, cs.sequence AS seq
                      FROM {course_sections} AS cs
                     WHERE cs.course = ?
                  ORDER BY cs.section;";
            $params   = [];
            $params[] = $this->courseid;
            if ($originalsort = $DB->get_records_sql($sql, $params)) {
                $cmseq = [];
                foreach ($originalsort as $section) {
                    $sections = explode(',', $section->seq);
                    $cmseq    = array_merge($cmseq, $sections);
                }
                list($idssql, $idsparams) = $DB->get_in_or_equal($modulenames, SQL_PARAMS_NAMED);
                $sql = "SELECT name, id
                          FROM {modules} AS m
                         WHERE name $idssql";
                $moduleids = $DB->get_records_sql($sql, $idsparams);
                $wheresql  = '';
                foreach ($iteminstances as $iik => $ii) {
                    $moduleid = $moduleids[$ii[1]]->id;
                    $iiid     = $ii[0];
                    $or       = $wheresql ? " || " : '';
                    $wheresql .= $or . " (instance = $iiid AND module = $moduleid) ";
                    $iteminstances[$iik][1] = $moduleid;
                }
                $sql = "SELECT *
                          FROM {course_modules} AS cm
                         WHERE $wheresql";
                if ($cms = $DB->get_records_sql($sql)) {
                    $cmseq = array_flip($cmseq);
                    uasort($cms, function ($a, $b) use ($cmseq) {
                        if ($cmseq[$a->id] == $cmseq[$b->id]) {
                            return 0;
                        }
                        return ($cmseq[$a->id] < $cmseq[$b->id]) ? -1 : 1;
                    });
                    asort($sortkeys);
                    foreach ($cms as $id => $cm) {
                        $oldsortkey = multi_array_search($iteminstances, [0 => $cm->instance, 1 => $cm->module]);
                        $sortorder = array_shift($sortkeys);
                        $params = array($sortorder, $iteminstances[$oldsortkey[0]][2]);
