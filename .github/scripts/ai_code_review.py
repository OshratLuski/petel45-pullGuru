...
def parse_comments_json(text: str) -> list[dict]:
    try:
        ...
        if start != -1 and end != -1 and end > start:
            ...
            return []
    # Accept both dict-with-comments and a raw list
    if isinstance(obj, dict):
        return obj.get("comments", []) or []
...
def find_new_line_fuzzy(patch: str, anchor_text: str) -> int | None:
    """
    Match anchors ONLY to added '+' lines. If anchor corresponds to a '-' (deletion),
    bind to the nearest '+' in the same hunk (replacement).
    """
    if not anchor_text:
        return None
    ...
    # 1) exact match on added line only
    for idx, ln in enumerate(lines, 1):
        if ln.startswith('+') and ln[1:] == target_exact:
            return m.get(idx)

    # 2) normalized equality on added line only
    for idx, ln in enumerate(lines, 1):
        if ln.startswith('+') and normalize_code_line(ln[1:]) == target_norm:
            return m.get(idx)

    # 3) substring fallback on added line only
    if len(target_norm) >= 6:
        for idx, ln in enumerate(lines, 1):
            if ln.startswith('+') and target_norm in normalize_code_line(ln[1:]):
                return m.get(idx)
...
def sanitize_suggestion(s: str) -> str:
    """Keep only the new code for GitHub suggestions (strip '-' lines, drop '+' prefixes)."""
    if not s:
        return s
    out = []
    for line in s.splitlines():
        if line.startswith('-'):
            continue  # drop old code
        if line.startswith('+'):
            out.append(line[1:])  # keep original indentation
        else:
            out.append(line)
...
def main():
    ...
    for c in raw_comments:
        ...
        body = msg
        if suggestion:
            cleaned = sanitize_suggestion(suggestion.rstrip("\n"))
            if cleaned:
                body += "\n\n```suggestion\n" + cleaned + "\n```"
        
        key = (path, new_line, normalize_code_line(msg)[:160])
        if key in seen:
            debug(f"duplicate comment suppressed: {path}:{new_line}")
            continue
        seen.add(key)
    ...
    note = ""
    if published == 0:
        note = "\n\n_No inline comments were posted; see counters above._"
