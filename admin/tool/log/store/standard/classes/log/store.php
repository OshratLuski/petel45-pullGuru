...
class store implements \tool_log\log\writer, \core\log\sql_internal_table_reader {
    ...
    protected function insert_event_entries($evententries) {
        ...
        foreach ($evententries as $key => $evententry) {
            if (isset($evententry['other']) && !empty($evententry['other'])) {
                $value = json_decode($evententry['other']);
                $evententries[$key]['other'] = json_encode($value, JSON_UNESCAPED_UNICODE);
            }
        }
