...
class users extends system_report {
    ...
    public function add_columns(): void {
        ...
        if (!in_array('user:idnumber', array_keys($this->get_columns()))) {
            $this->add_column($entityuser->get_column('idnumber'));
        }
    ...
    protected function add_actions(): void {
        ...
        $this->add_action((new action(
                ...
                [],
                false,
                new lang_string('loginas', 'moodle'),
        ))->add_callback(static function(\stdclass $row) use ($USER, $contextsystem): bool {
            return has_capability('moodle/user:loginas', $contextsystem) &&
                    !is_mnet_remote_user($row) && $row->id != $USER->id && !is_siteadmin($row);
        }));

        // Action to usersroles.
        $this->add_action((new action(
                new moodle_url('/admin/roles/usersroles.php', ['courseid' => 1, 'userid' => ':id', 'sesskey' => sesskey()]),
                new pix_icon('t/assignroles', ''),
                [],
                false,
                new lang_string('roles', 'moodle'),
        ))->add_callback(static function(\stdclass $row) use ($USER, $contextsystem): bool {
            return !is_mnet_remote_user($row) && $row->id != $USER->id && !is_siteadmin($row);
        }));
    }
