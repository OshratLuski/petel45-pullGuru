...
class question_attempt {
    ...
    public function get_attempt_state(){
        ...
        if(!empty($attempt)){
            ...
            if ($attempt->state == \mod_quiz\quiz_attempt::IN_PROGRESS) {
                return \mod_quiz\question\display_options::DURING;
            } else if ($quiz->timeclose && time() >= $quiz->timeclose) {
                return \mod_quiz\question\display_options::AFTER_CLOSE;
            } else if (time() < $attempt->timefinish + 120) {
                return \mod_quiz\question\display_options::IMMEDIATELY_AFTER;
            } else {
                return \mod_quiz\question\display_options::LATER_WHILE_OPEN;
            }
        }else{
            return \mod_quiz\question\display_options::LATER_WHILE_OPEN;
        }
    ...
    public function get_quiz_options($part = null){
        ...
        switch ($state) {
            case \mod_quiz\question\display_options::DURING:
                $whenname = 'during';
                $when = \mod_quiz\question\display_options::DURING;
                break;
            case \mod_quiz\question\display_options::IMMEDIATELY_AFTER:
                $whenname = 'immediately';
                $when = \mod_quiz\question\display_options::IMMEDIATELY_AFTER;
                break;
            case \mod_quiz\question\display_options::LATER_WHILE_OPEN:
                $whenname = 'open';
                $when = \mod_quiz\question\display_options::LATER_WHILE_OPEN;
                break;
            case \mod_quiz\question\display_options::AFTER_CLOSE:
                $whenname = 'closed';
                $when = \mod_quiz\question\display_options::AFTER_CLOSE;
                break;
            default:
                $whenname = '';
                $when = 0;
        ...
        if($part != null) {
            if (!$this->if_user_answered_on_part($part) && $when == mod_quiz\question\display_options::DURING) {
