...
class qtype_savpl_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        ...
        $mform->setDefault('disablerun', 1);
        $mform->addElement('advcheckbox', 'disableevaluate', null, get_string('disableevaluate', SAQVPL));
        $mform->setType('disableevaluate', PARAM_INT);
        $mform->setDefault('disableevaluate', 0);
        $mform->addElement('advcheckbox', 'trywithoutgrade', null, get_string('trywithoutgrade', SAQVPL));
        $mform->setType('trywithoutgrade', PARAM_INT);
        $mform->setDefault('trywithoutgrade', 0);

        $mform->addElement('advcheckbox', 'aisupport', null, get_string('aisupport', SAQVPL));
        $mform->setType('aisupport', PARAM_INT);

        $mform->addElement('text', 'ainumrequests', get_string('ainumrequests', SAQVPL), array('size' => '3'));
        $mform->setType('ainumrequests', PARAM_INT);

        $mform->addElement('textarea', 'aiteacherprompt', get_string('aiteacherprompt', SAQVPL));
    ...
    public function definition_after_data() {
        ...
        if (!empty($this->_form->getElementValue('trywithoutgrade'))) {
            $questionid = $this->question->id;

            // Retrieve the quiz slot associated with the question ID.
            $sql = "SELECT slot.*
            ...
            $quizslot = $DB->get_records_sql($sql, $params);
            $quizslot = reset($quizslot);

            if ($quizslot->maxmark != 0) {
                $sql2 = "UPDATE {quiz_slots}
                SET maxmark = 0.0
                WHERE id = :id";

                $params2 = ['id' =>$quizslot->id];

                $DB->execute($sql2, $params2);

                \mod_quiz\quiz_settings::create($quizslot->quizid)->get_grade_calculator()->recompute_quiz_sumgrades();
            }
        }
    }
