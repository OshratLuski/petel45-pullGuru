...
function groupBBFix(selected) {
  ...
  if(ref) {
    ...
    matched = copy.filter('line, arrow, path');
  } else {
    matched = $(selected).find('line, arrow, path');
  }
...
svgedit.utilities.getBBox = function(elem) {
  ...
  default:
    ...
    if(elname === 'use') {
      ...
      //  ret = bb;
      //}
    } else if (elname === 'arrow') {
      const x1 = parseFloat(selected.getAttribute('x1')) || 0;
      const y1 = parseFloat(selected.getAttribute('y1')) || 0;
      const x2 = parseFloat(selected.getAttribute('x2')) || 0;
      const y2 = parseFloat(selected.getAttribute('y2')) || 0;

      const markerSize = 10;
      ret = {
        x: Math.min(x1, x2) - markerSize,
        y: Math.min(y1, y2) - markerSize,
        width: Math.abs(x2 - x1) + markerSize * 2,
        height: Math.abs(y2 - y1) + markerSize * 2,
      };
