...
/*
var svgcontentx = svgdoc.createElementNS(svgns, "g");
$(svgcontentx).attr({
  id: 'goooooooooooo',
}).appendTo(svgcontent);
*/
  ...
  var comment = svgdoc.createComment(qtype_drawing_str_comment);
...
var visElems = 'a,circle,ellipse,foreignObject,g,image,line,arrow,path,polygon,polyline,rect,svg,text,tspan,use';
  ...
  var getCheckedBBox = function(elem) {
    ...
    try {
      ...
      if ((angle && angle % 90) ||
        ...
        var elemNames = ['ellipse','path','line', 'arrow','polyline','polygon'];
...
/*
if(!elemsToAdd || elemsToAdd == null){ //added by amr to cover for FHPATH.
  return;
}*/
...
// Function: addToSelectionFhpath
// Adds a list of elements to the selection.  The 'selected' handler is then called.
//
// Parameters:
// elemsToAdd - an array of DOM elements to add to the selection
// showGrips - a boolean flag indicating whether the resize grips should be shown
var addToSelectionFhpath = this.addToSelectionFhpath = function(elemsToAdd, showGrips) {
  if (elemsToAdd.length == 0) { return; }
  // find the first null in our selectedElements array
  var j = 0;
...
// Function: SpecialFhpathselectOnly()
// Selects only the given elements, shortcut for clearSelection();
//
// Parameters:
// elems - an array of DOM elements to be selected
var SpecialFhpathselectOnly = this.SpecialFhpathselectOnly = function(elems, showGrips) {
  clearSelection(true);
  addToSelectionFhpath(elems, false);
}
...
//          console.log('s',[start_x, start_y]);
      ...
      case "line":
        ...
        case "arrow":
          started = true;
          // Ensure the arrowhead marker is defined in <defs>, creating it if missing.
          var defs = findDefs();
          if (defs && !document.getElementById("arrowhead")) {
              var arrowMarker = svgdoc.createElementNS(svgns, "marker");
              ...
              var arrowPolygon = svgdoc.createElementNS(svgns, "polygon");
              arrowPolygon.setAttribute("points", "0,0 10,3.5 0,7");
              arrowPolygon.setAttribute("fill", cur_shape.stroke || "black");
...
//              call("selected", [selected]);
      ...
      case "fhpath":
        ...
        // AMR try to create a dot when length is just a click (draw dot!)
        var points = element.points;
        if (points.numberOfItems <= 4) {
          //Splitting it with : as the separator
          var myarr = coords.split(",");
          var myvar1 = parseFloat(myarr[0])+1;
          var myvar2 = parseFloat(myarr[1])+1;
...
var textActions = canvas.textActions = function() {
  ...
  function setCursor(index) { return;
  ...
  function setSelection(start, end, skipInput) { return;
    if(start === end) {
      setCursor(end);
      return;
    }
...
//        }
  ...
  //   if(elem.firstChild) textinput = elem.firstChild;
      textinput = elem;
...
//        }
    ...
    //  var str = (curtext.textContent).trim();
      var str = curtext.textContent;
      var len = str.length;
  ...
  evt.currentTarget.innerHTML = originaltxt;
   ...
   // $(curtext).unbind('doubletap', selectWord).doubletap(selectWord);
      ...
      for(var i=0; i<len; i++) {
        ...
        chardata[i] = {
          x: start.x,
          y: start.y, // start.y? //was textbb.y
          width: end.x - start.x,
...
this.setSvgStringEraser = function(xmlString) {
...
//  var new_layer = getCurrentDrawing().createLayer('Eraser');
...
this.setMode = function(name) {
  ...
  if(selectedElements[0]){
    if(selectedElements[0].nodeName != 'text'){
      $('#strokewidth_div').show();
      $('#strokestyle_div').show();
    } else{
      $('#strokewidth_div').hide();
...
this.resetStrokeWidth = function(val) {
...
this.setStrokeWidth = function(val) {
  if(val == 0 && ['line', 'arrow', 'path'].indexOf(current_mode) >= 0) {
...
// Parameters:
// val - Float with the new font size
this.resetFontSize = function(val) {
cur_text.font_size = val;
changeSelectedAttribute("font-size", val);
};
...
this.notifyTextchange = function(val, e) {
...
this.setTextContent = function(val, e) {
  ...
  var newtextlines = val.split(/\n/);
   var updatedtxtval = '';
   var extrady ='';
   var textXlocation = selectedElements[0].getAttribute('x');
   var textYlocation = selectedElements[0].getAttribute('y');
   var textid = selectedElements[0].getAttribute('id');
...
var changeSelectedAttributeNoUndo = this.changeSelectedAttributeNoUndo = function(attr, newValue, elems) {
    ...
    while (i--) {
      ...
      if(current_mode === "textedit" && attr !== "text" && elem.innerHTML.length) {
      ...
      var oldval = attr === "text" ? elem.innerHTML : elem.getAttribute(attr);
...
this.setFHDBackground = setFHDBackground = function (im){
  ...
  if (whichbackground != ''){
    ...
    image.onload = function () {
      ...
      svgCanvas.setBackground("",whichbackground);
