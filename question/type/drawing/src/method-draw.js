...
(function() {
  ...
  if(!window.methodDraw) window.methodDraw = function($) {
    ...
    Editor.init = function() {
      ...
      curConfig.dimensions =  [$('#fhd_width').val(), $('#fhd_height').val()];

/*
      questionID = $('#fhd_question_id').val();
      fhd_width = $('#fhd_width').val();
      fhd_height = $('#fhd_height').val();
      $("#canvas_width").val(fhd_width);
      $("#canvas_height").val(fhd_height);
      $("#canvas_width").trigger('change');
      $("#canvas_height").trigger('change');
      console.error(fhd_height);
      */
...
//console.error("fhd_display_mode",'#' + el_name + '_panel', fhd_display_mode);
            ...
            if(el_name == 'text') {
              ...
              $('#text').val(elem.innerHTML); //textContent
        ...
        //added by amr for disabling undo first time.
        if(undoMgr.getNextUndoCommandText() == 'Create Layer' || undoMgr.getNextUndoCommandText() == ''){
          $('#tool_undo').addClass( 'disabled');
        }
        ...
        //added by amr to disabled fillbucket when no object selected.
      ...
      var changeStrokeWidth = function(ctl) {
        var val = ctl.value;
        if(val == 0 && selectedElement && ['line', 'polyline', 'arrow'].indexOf(selectedElement.nodeName) >= 0) {
          val = ctl.value = 1;
        }
        if(svgCanvas.getMode() == 'fhpath' || svgCanvas.getMode() == 'line' || svgCanvas.getMode() == 'arrow'){ // dont allow stroke for drawing to go zero so it draws.
          if(val == 0){
            val = ctl.value = 3.5;
          }
        }
...
*/
      ...
      $('#text').keyup(function(e){
        ...
        var newtextlines = txtval.split(/\n/);
         ...
         var textYlocation = sel[0].getAttribute('y');
         var textid = sel[0].getAttribute('id');
           for(var i=0;i<newtextlines.length;i++){
            ...
            sel[0].innerHTML = updatedtxtval;
            svgCanvas.selectorManager.requestSelector(sel[0]).resize();

        //    svgCanvas.changeSelectedAttributeNoUndo("text", updatedtxtval, sel);
      ...
      */
      ...
      var clickLineOrArrowTool = function(mode, buttonId) {
        if (toolButtonClick(buttonId)) {
          svgCanvas.setMode(mode);
          if (!$('#stroke_width').val() || $('#stroke_width').val() <= 0) {
            $('#stroke_width').val(3.5); // Stroke should not have an initial effect.
            svgCanvas.setStrokeWidth(3.5);
          }
          $('#preset_sizes_panel_id').show();
          if (fhd_display_mode == 1) {
      ...
      var clickLine = function() {
        clickLineOrArrowTool('line', '#tool_line');
      };
      
      var clickArrow = function() {
        clickLineOrArrowTool('arrow', '#tool_arrow');
      };
      ...
      var quicksizechange = function(e) {
          var size = $(this).data('size');
/*
...
*/
        //  var penortext =  $(this).data('penortext');
...
*/
          ...
          $(".qtype_drawing_size_pen div").removeClass("qtype_drawing_active_selection");
          $("#"+this.id+" div").addClass("qtype_drawing_active_selection");
          //$('#qtype_drawing_tool_pensize1 div').addClass("qtype_drawing_active_selection");

      };

      //Function to convert rgb color to hex format
      var clickquickcolor = function(e) {
          ...
          // Stroke or fill? for pen or line it must be stroke!
          var picker = $(this).data('strokeorfill');
          var rgbcolor =  $(this).css('backgroundColor');
          var paint = null;
          var noUndo = true;
          var hexcolor =  rgb2hex(rgbcolor);
          paint = new $.jGraduate.Paint({alpha: 100, solidColor: hexcolor.substr(1)});
...
*/
          if (hexcolor != 'none' && svgCanvas.getStrokeOpacity() != 1) {
           ...
           // $("#tool_fhpath").css("background-color", rgbcolor);
          // remove highlight from all others!
          $(".qtype_drawing_color_divsquare").removeClass("qtype_drawing_selectedcolor");
          $(".qtype_drawing_color_pallette").removeClass("qtype_drawing_selectedcolor");
          $("#"+this.id).addClass("qtype_drawing_selectedcolor");

      };

      var quickcolorpallette = function(e){
        var picker = $(this).data('strokeorfill');
        var anticipateddiv = '#' + picker + '_color';
        //picker
        colorPicker($(anticipateddiv));
        $(".qtype_drawing_color_divsquare").removeClass("qtype_drawing_selectedcolor");
        $(".qtype_drawing_color_pallette").removeClass("qtype_drawing_selectedcolor");
      //  $("#"+this.id).addClass("qtype_drawing_selectedcolor");
      ...
      var clickClear = function(){
        ...
        $.confirm(qtype_drawing_str_newconfirmationmsg, function(ok) {
          if(!ok) return;
          setSelectMode();
          svgCanvas.deleteSelectedElements();
          svgCanvas.clear();
      ...
      var clickFill = function(){
        if(!$('#tool_bucket').hasClass('disabled')){
    ...
    //    updateBgImage();
      ...
      var minimizeModal = function() {
        ...
        //  top.exit_fullscreen();
        $("#text_dialog_jq").dialog("close");
    //  $( "#textedit_dialog" ).dialog().close();
      ...
      var saveSourceEditor = function(){
        ...
        if (!svgCanvas.setSvgString($('#svg_source_textarea').val())) {
          $.confirm(qtype_drawing_str_parsingerror, function(ok) {
            if(!ok) return false;
            saveChanges();
          });
        } else {
      ...
      var cancelOverlays = function() {
        ...
        if (editingsource) {
          if (orig_source !== $('#svg_source_textarea').val()) {
            $.confirm(qtype_drawing_str_ignorechanges, function(ok) {
...
/*
          if(this.type == 'stroke'){
            curConfig.globalstrokecolor = fillAttr;
          } else {
              curConfig.globalfillcolor = fillAttr;
          }*/
      //    var whichcolortypepaint = 'global' + this.type + 'color';
        //  curConfig.whichcolortypepaint = fillAttr;
      //    curConfig.globalstrokecolor = fillAttr;
        ...
        $("#qtype_drawing_tool_color15 > img").css("background-color", fill_color);//.css("outline", "1px "+curConfig.globalstrokecolor+" solid!important");
          ...
          if (this.type == "canvas") {
            ...
            if (background) {
              ...
              background.setAttribute("width", res.w + 2); //res.w+2
    ...
    var changeCanvasSize = function(ctl){
      ...
      var Actions = function() {
        ...
        var tool_buttons = [
          ...
          {sel:'#tool_arrow', fn: clickArrow, evt: 'click', key: ['B', true]},
          ...
          // Preset Sizes - Amr
          {sel:'.qtype_drawing_size_pen', fn: quicksizechange, evt: 'click'},
          // Preset Colors - Amr
          {sel:'.qtype_drawing_color_divsquare', fn: clickquickcolor, evt: 'click'},
          {sel:'.qtype_drawing_color_pallette', fn: quickcolorpallette, evt: 'click'},
          ...
          {sel:'#tool_bucket', fn: clickFill, evt: 'click'},
      ...
      $('#arrow_x1')      .dragInput({ min: null, max: null,  step:  1,  callback: changeAttribute,     cursor: false                         });
      $('#arrow_x2')      .dragInput({ min: null, max: null,  step:  1,  callback: changeAttribute,     cursor: false                         });
      $('#arrow_y1')      .dragInput({ min: null, max: null,  step:  1,  callback: changeAttribute,     cursor: false                         });
      $('#arrow_y2')      .dragInput({ min: null, max: null,  step:  1,  callback: changeAttribute,     cursor: false                         });
      ...
      Editor.openPrep = function(func) {
        ...
        if(undoMgr.getUndoStackSize() === 0) {
          ...
          $.confirm(qtype_drawing_str_newconfirmationmsg, func);
        }
      }
...
//      $(function() {
        updateCanvas(true);
        // added by amr for initial state.
//      });
  ...
  //    if(Editor.numsaved > 2){
           ...
           var arrow =  gpaths.selectAll("arrow").node();
           ...
           if(Editor.forcesaving == 1 || (polyline && polyline != null) || (path && path != null) || (line && line != null) || (arrow && arrow != null) || (polygon && polygon != null) || (rect && rect != null) || (text && text != null) || (ellipse && ellipse != null)){
