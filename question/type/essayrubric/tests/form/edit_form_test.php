...
namespace qtype_essayrubric\form;
...
require_once($CFG->dirroot . '/question/type/essayrubric/edit_essayrubric_form.php');

/**
 * Unit tests for the essayrubric edit form.
 *
...
class edit_form_test extends \advanced_testcase {
    ...
    protected function get_form($classname) {
        ...
        $fakequestion->qtype = 'essayrubric';
    ...
    public function test_attachments_validation(int $allowed, int $required, array $expected): void {
        list($form, $category) = $this->get_form('qtype_essayrubric_edit_form');
        $submitteddata = [
    ...
    public function user_preference_provider(): array {
        $valid = [];
        $invalid = ['attachmentsrequired' => get_string('mustrequirefewer', 'qtype_essayrubric')];
        return [
