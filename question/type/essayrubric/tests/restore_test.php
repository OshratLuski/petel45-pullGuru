...
class restore_test extends \restore_date_testcase {
    ...
    /**
     * Test missing qtype_essayrubric_options creation.
     *
     * Old backup files may contain essays with no qtype_essayrubric_options record.
     * During restore, we add default options for any questions like that.
     * That is what is tested in this file.
     */
    public function test_restore_create_missing_qtype_essayrubric_options() {
        global $DB;

        // Create a course with one essayrubric question in its question bank.
        ...
        $essayrubric = $questiongenerator->create_question('essayrubric', null, array('category' => $category->id));

        // Remove the options record, which means that the backup will look like a backup made in an old Moodle.
        $DB->delete_records('qtype_essayrubric_options', ['questionid' => $essayrubric->id]);
        ...
        $newessay = $DB->get_record_sql('SELECT q.*
                                             ...
                                             WHERE qbe.questioncategoryid = ?
                                               AND q.qtype = ?', [$newcategory->id, 'essayrubric']);
        $this->assertTrue($DB->record_exists('qtype_essayrubric_options', ['questionid' => $newessay->id]));
    }
}
