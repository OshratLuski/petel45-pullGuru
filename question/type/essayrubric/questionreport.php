 ...
 */
 ...
 $title = get_string('questionreport', 'qtype_essayrubric');
 ...
 $PAGE->set_url(new moodle_url('/question/type/essayrubric/questionreport.php'), $urlparams);
 ...
 if (!has_capability('moodle/course:update', $context, $USER)) {
     echo $OUTPUT->notification(get_string('nopermissiontoaccesspage', 'qtype_essayrubric'));
     echo $OUTPUT->footer();
     return;
 }
 ...
 echo $OUTPUT->render_from_template('qtype_essayrubric/attempts_report', $data);
 
 $PAGE->requires->js_call_amd('qtype_essayrubric/qreport', 'init', [$questionid, (object) ['last_page' => $data->last_page]]);
