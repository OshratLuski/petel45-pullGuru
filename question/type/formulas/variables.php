...
class variables {
    ...
    public function instantiate_random_variables(&$vstack, $datasetid = -1, $fixedmin = false, $random = false) {
        ...
        foreach ($vstack->all as $name => $data) {
            if ( $data->type[0] == 'z') {
                ...
                if ( $data->type[1] == 'h') {
                    ...
                    if ($data->type[1] == 'n') {
                        if ($fixedmin) {
                            if (isset($v->elements[0][0])) {
                                $this->vstack_update_variable($newstack, $name, null, 'n', $v->elements[0][0]);
                            }
                            continue;
                        }
                        ...
                        foreach ($v->elements as $elem) {
                            ...
                            if ($random) {
                                $randindex = mt_rand(0, $sz - 1);
                                $value = $elem[0] + $randindex * $elem[2];
                                $this->vstack_update_variable($newstack, $name, null, 'n', $value);
                                break;
                            }
    
                            if ($id < $sz) {
                                $value = $elem[0] + $id * $elem[2];
                                $this->vstack_update_variable($newstack, $name, null, 'n', $value);
                                break;
                            }
