...
class qtype_formulas_question extends question_graded_automatically_with_countback
    ...
    public function start_attempt(question_attempt_step $step, $variant) {
        ...
        try {
            ...
            $datasetid = -1;
            $fixedmin = $random = false;

            if ($cmid) {
                // Get role.
                $flagadmin = false;
                foreach (get_user_roles(\context_course::instance($COURSE->id), $step->get_user_id()) as $role) {
                    if (in_array($role->shortname, ['manager', 'teacher', 'editingteacher'])) {
                        $flagadmin = true;
                    }
                }
                ...
                // Check numdataset.
                $numdataset = $this->qv->vstack_get_number_of_dataset($vstack);
                if ($numdataset == null || $numdataset == 0) {
                    $flagadmin = true;
                }

                if (!$flagadmin) {
                    $variable = $this->questionbankentryid . $step->get_user_id();

                    // Start new attempt.
                    if (!$attemptid) {
                        $SESSION->$variable = rand(1, 100);
                        $datasetid = $SESSION->$variable;
                        $random = true;
                    } else {
                        // Start reattempt.
                        $datasetid = $SESSION->$variable;
                ...
                $lqsoptions = \local_quiz_summary_option\funcs::get_quiz_config($cmid);
                if ($lqsoptions->summary_fixedmin) {
                    $fixedmin = true;
                }
            }

            $this->randomsvars = $this->qv->instantiate_random_variables($vstack, $datasetid, $fixedmin, $random);
    ...
    public function grade_response(array $response) {
        ...
        foreach ($this->parts as $part) {
            ...
            $this->fractions[$part->partindex] = $this->anscorrs[$part->partindex] * ($this->unitcorrs[$part->partindex] >= 1 ? 1 : $this->unitcorrs[$part->partindex]);
    ...
    public function grade_responses_individually($part, $response, &$checkunit, $forvalidation = false) {
        ...
        if ($this->is_autocomplete_state($part)) {
            ...
            if (!method_exists($part,'part_has_combined_unit_field')) {
                return array(0, 0);
            }
            ...
            if ($autocomplete->compare_answer()) {
                ...
                if ($obj->result === true) {
                    // Ensure penalty is numeric and within valid range [0,1].
                    $penalty = min(floatval($obj->penalty), 1);

                    if ($obj->penaltytype === 'value') {
                        // Penalty is due to incorrect value only.
                        $answercorrect = 1 - $penalty;
                        $unitcorrect = 1;
                        $part->feedback = get_string('feedbackwrongvalue', 'qtype_formulas');

                    } elseif ($obj->penaltytype === 'unit') {
                        // Penalty is due to incorrect unit only.
                        $answercorrect = 1;
                        $unitcorrect = 1 - $penalty;
                        $part->feedback = get_string('feedbackwrongunit', 'qtype_formulas');
                    }
                } else {
                    // Answer is incorrect with no partial credit.
    ...
    public function compute_final_grade($responses, $totaltries) {
        ...
        foreach ($this->parts as $part) {
            ...
            foreach ($responses as $responseindex => $response) {
                ...
                // Both $anscorrs and $unitcorrs are float values already including penalties.
                // We multiply them directly to compute the final part fraction.
                $partfraction = $anscorrs * $unitcorrs;
            }
