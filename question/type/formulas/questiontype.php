...
class qtype_formulas extends question_type {
        ...
        try {
            ...
            foreach ($newanswers as $i => $ans) {
                ...
                if (!$answer) {
                    ...
                    // Ensure the 'numbox' field is set to 1 if it is missing or invalid (e.g., during restore or copy).
                    // Without this, the renderer fails to create input fields for the student's answer.
                    if (!isset($answer->numbox) || (int)$answer->numbox < 1) {
                        $answer->numbox = 1;
                        $ans->id = $DB->update_record('qtype_formulas_answers', $answer);
                    }
                }
    ...
    public function validate_instantiation($form, &$validanswers) {
        ...
        foreach ($validanswers as $idx => $ans) {
            ...
            if ($ans->autocomplete == 1 && $ans->answertype == 0) {
                break;
            }
