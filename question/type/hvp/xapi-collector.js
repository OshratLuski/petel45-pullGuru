...
(function($) {
    ...
    function storeXAPIData(contentId, event) {
        ...
        var data = {
            contentId: contentId,
            questionattemptid: H5PIntegration.contents['cid-' + instance.contentId].qaid,
    ...
    $(document).ready(function() {
        ...
        H5P.externalDispatcher.on('xAPI', function(event) {
            ...
            var statementsVerbs = H5PIntegration.statements;
            var statements = statementsVerbs.split(",");
            var arrStatments = [];

            statements.forEach(function (statment) {
                arrStatments.push(statment.trim());
            });

            var isCompleted = arrStatments.includes(statement.verb.display['en-US']);
