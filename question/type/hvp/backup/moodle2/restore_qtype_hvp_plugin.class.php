    ...
    public function process_hvp($data) {
        ...
        if ($questioncreated) {
            ...
            if ($maxfileid == 0) {
                ...
                $this->add_related_files('qtype_hvp', 'editor', 0);

                // Add hvp related files.
                $maxfileid = $DB->get_field_sql("SELECT MAX(id) FROM {files}");
            }

            $newfiles = $DB->get_records_sql("SELECT * FROM {files} WHERE id > ? AND component=? AND filearea IN('content', 'editor') AND itemid=?",
                    [$maxfileid, 'qtype_hvp', $oldid]);
