...
class qtype_hvp_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        ...
        $mform->addElement('header', 'styles', get_string('styles', 'qtype_hvp'));
        $mform->addElement('textarea', 'css', get_string('css', 'qtype_hvp'),
    ...
    public function data_preprocessing($question) {
        ...
        if (!empty($question->options)) {
            $question->css = $question->options->hvp['css'];
        }
        $mformid = $this->_form->getAttribute('id');
        $itemid = !empty($question->id) ? $question->id : 0;
        \hvp_add_editor_assets($question, $mformid, $itemid);
        return $question;
    }
