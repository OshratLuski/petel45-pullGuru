...
class qtype_hvp extends question_type {
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        if (!empty($content)) {
            ...
            $fs = get_file_storage();
            $editorfiles = $fs->get_area_files($question->contextid, 'qtype_hvp', 'editor', 0);
            $contentfiles = $fs->get_area_files($question->contextid, 'qtype_hvp', 'content', $question->id);
            $expout .= "    <editorfiles>\n";
            $expout .= $format->write_files($editorfiles);
            $expout .= "    </editorfiles>\n";

            $expout .= "    <contentfiles>\n";
            $expout .= $format->write_files($contentfiles);
            $expout .= "    </contentfiles>\n";
    ...
    public function save_question_options($question) {
        ...
        // TODO: get proper module (?) context
        $contextid = $question->context->id;

        // Move files from draft area to final location
        if (!empty($question->editorfiles)) {
            $this->save_draft_files($question, $contextid, $question->editorfiles, 0);
        }
        if (!empty($question->contentfiles)) {
            $this->save_draft_files($question, $contextid, $question->contentfiles, $question->id);
        }

        parent::save_question_options($question);
    }

    protected function save_draft_files($question, $contextid, $filetype, $itemid) {
        global $USER;
        ...
        if (!$draftfiles = $fs->get_area_files($usercontext->id, 'user', 'draft', $filetype, 'id DESC', false)) {
            return null;
        }

        foreach ($draftfiles as $draftfile) {
            // Prepare the file record for saving the draft file as a component file
            $filerecord = [
                'contextid' => $contextid, // The context ID of the question
                'component' => 'qtype_hvp', // The component name
                'filearea' => $itemid == 0 ? 'editor' : 'content', // The file area name
                'itemid' => $itemid, // The question ID
                'filepath' => '/images/', // The file path
                'filename' => $draftfile->get_filename(), // The file name
            ];
            ...
            if (!$fs->file_exists($filerecord['contextid'], $filerecord['component'], $filerecord['filearea'], $filerecord['itemid'], $filerecord['filepath'], $filerecord['filename'])) {
                // Save the draft file to the component's file area
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra = null) {
        ...
        if ($data['@']['type'] == 'hvp') {
            ...
            $filexml = $format->getpath($data['#']['editorfiles'][0], array('#', 'file'), array());
            $qo->editorfiles = $format->import_files_as_draft($filexml);
            $filexml = $format->getpath($data['#']['contentfiles'][0], array('#', 'file'), array());
            $qo->contentfiles = $format->import_files_as_draft($filexml);
