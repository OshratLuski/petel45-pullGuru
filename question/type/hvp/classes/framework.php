...
require_once($CFG->dirroot . '/question/type/hvp/library/h5p-metadata.class.php');
require_once($CFG->dirroot . '/question/type/hvp/editor/h5peditor.class.php');


use \qtype_hvp_library\H5PCore as H5PCore;
use \qtype_hvp_library\H5PContentValidator as H5PContentValidator;
use \qtype_hvp_library\H5PStorage as H5PStorage;
use \qtype_hvp_library\H5PValidator as H5PValidator;
use \qtype_hvp_library\H5PPermission as H5PPermission;
use \qtype_hvp_library\H5PContentHubSyncStatus as H5PContentHubSyncStatus;
 ...
 * @package    qtype_hvp
 */
class framework  implements \qtype_hvp_library\H5PFrameworkInterface {
    ...
    public static function instance($type = null) {
        ...
        if (!isset($interface)) {
            $interface = new \qtype_hvp\framework();

            // Support alternate file storage class defined in $CFG.
            if (!empty($CFG->qtype_hvp_file_storage_class)) {
                $fsclass = $CFG->qtype_hvp_file_storage_class;
            } else {
                $fsclass = '\qtype_hvp\file_storage';
            }
            ...
            $url = "{$root}/pluginfile.php/{$context->id}/qtype_hvp";
        ...
        switch ($type) {
            ...
            case 'editor':
                if (empty($editorinterface)) {
                    $editorinterface = new \qtype_hvp\editor_framework();
                }

                if (empty($editorajaxinterface)) {
                    $editorajaxinterface = new  \qtype_hvp\editor_ajax();
                }

                if (empty($editor)) {
                    $editor = new \qtype_hvp\editor\H5peditor($core, $editorinterface, $editorajaxinterface);
                }
                return $editor;
            case 'core':
            default:
    ...
    public function loadContent($id) {
        ...
        $data = $DB->get_record_sql("
          SELECT
            ...
            hc.a11y_title
          FROM {qtype_hvp} hc
          JOIN {qtype_hvp_libraries} hl ON hl.id = hc.main_library_id
          WHERE hc.question = ?", array($id)
        );
...
//        $data->name = "NAME PLACE HOLDER";
    ...
    public function updateContent($content, $contentmainid = null) {
        ...
        if (!isset($content['id'])) {
            ...
            if ($obj = $DB->get_record('qtype_hvp', ['question' => $data['question']])) {
                $data['id'] = $obj->id;
                $DB->update_record('qtype_hvp', $data);
                $id = $data['id'];
            } else {
                $id = $DB->insert_record('qtype_hvp', $data);
            }
        } else {
            $data['id'] =  $content['id'];
    ...
    public function getLibraryUsage($id, $skipcontent = false) {
        ...
        if ($skipcontent) {
            ...
            $content = intval($DB->get_field_sql(
                ...
                FROM {qtype_hvp_libraries} l
                JOIN {qtype_hvp_contents_libraries} cl ON l.id = cl.library_id
                JOIN {qtype_hvp} c ON cl.hvp_id = c.id
                WHERE l.id = ?", array($id)
            ));
        }
    ...
    public function copyLibraryUsage($contentid, $copyfromid, $contentmainid = null) {
        ...
        $libraryusage = $DB->get_record('qtype_hvp_contents_libraries', array(
            'id' => $copyfromid
        ));

        $libraryusage->id = $contentid;
        $DB->insert_record_raw('qtype_hvp_contents_libraries', (array)$libraryusage, false, false, true);
    ...
    public function loadContentDependencies($id, $type = null) {
        ...
        $query = "SELECT hcl.id AS unidepid
                       ...
                       , hcl.drop_css
                       , hcl.dependency_type
                   FROM {qtype_hvp_contents_libraries} hcl
                   JOIN {qtype_hvp_libraries} hl ON hcl.library_id = hl.id
                  WHERE hcl.hvp_id = ?";
        $queryargs = array($id);
    ...
    public function deleteContentData($contentid) {
        ...
        $DB->delete_records('qtype_hvp_contents_user_data', array('hvp_id' => $contentid));
    }
    ...
    public function deleteLibraryUsage($contentid) {
        ...
        $DB->delete_records('qtype_hvp_contents_libraries', array('hvp_id' => $contentid));
    }
    ...
    public function saveLibraryUsage($contentid, $librariesinuse) {
        ...
        foreach ($librariesinuse as $dependency) {
            ...
            $DB->insert_record('qtype_hvp_contents_libraries', array(
                'hvp_id' => $contentid,
    ...
    public function getPlatformInfo() {
        ...
        return array(
            'name' => 'Moodle',
            'version' => $CFG->version,
            'h5pVersion' => \get_component_version('qtype_hvp'),
        );
    }
    ...
    public function t($message, $replacements = array()) {
        ...
        return get_string($translationsmap[$message], 'qtype_hvp', $replacements);
    }
    ...
    public function getH5pPath() {
        ...
        return $CFG->dirroot . '/qtype/hvp/files';
    }
    ...
    public function getLibraryFileUrl($libraryfoldername, $fileName) {
        ...
        return "{$basepath}pluginfile.php/{$context->id}/qtype_hvp/libraries/{$libraryfoldername}/{$fileName}";
    }
    ...
    public function loadAddons() {
        ...
        $records = $DB->get_records_sql(
                "SELECT l1.id AS library_id,
                        ...
                        l1.preloaded_js,
                        l1.preloaded_css
                   FROM {qtype_hvp_libraries} l1
              LEFT JOIN {qtype_hvp_libraries} l2
    ...
    public function getLibraryConfig($libraries = null) {
        global $CFG;
        return (isset($CFG->question_type_hvp_library_config) ? $CFG->question_type_hvp_library_config : null);
    }
    ...
    public function isPatchedLibrary($library) {
        ...
        if (isset($CFG->question_type_hvp_dev) && $CFG->question_type_dev) {
    ...
    public function saveLibraryData(&$librarydata, $new = true) {
        ...
        if ($new) {
            ...
            $library->id = $DB->insert_record('qtype_hvp_libraries', $library);
            ...
            // Save library data.
            $DB->update_record('qtype_hvp_libraries', (object) $library);
        ...
        $DB->delete_records('qtype_hvp_libs_languages', array('library_id' => $librarydata['libraryId']));
        if (isset($librarydata['language'])) {
            foreach ($librarydata['language'] as $languagecode => $languagejson) {
                $DB->insert_record('qtype_hvp_libs_languages', array(
    ...
    public function resetContentUserData($contentid) {
        ...
        $DB->execute("UPDATE {qtype_hvp_content_user_data}
                         SET data = 'RESET'
                       WHERE hvp_id = ?
    ...
    public function saveLibraryDependencies($libraryid, $dependencies, $dependencytype) {
        ...
        foreach ($dependencies as $dependency) {
            ...
            $dependencylibrary = $DB->get_record('qtype_hvp_libraries', array(
            ...
            // Create relation.
            $DB->insert_record('qtype_hvp_libs_libraries', array(
    ...
    public function loadLibrary($machinename, $majorversion, $minorversion) {
        ...
        $library = $DB->get_record('qtype_hvp_libraries', array(
        ...
        $dependencies = $DB->get_records_sql(
                'SELECT hl.id, hl.machine_name, hl.major_version, hl.minor_version, hll.dependency_type
                   FROM {qtype_hvp_libs_libraries} hll
                   JOIN {qtype_hvp_libraries} hl ON hll.required_library_id = hl.id
    ...
    public function alterLibrarySemantics(&$semantics, $name, $majorversion, $minorversion) {
        ...
        $renderer = $PAGE->get_renderer('qtype_hvp');
    ...
    public function deleteLibraryDependencies($libraryid) {
        ...
        $DB->delete_records('qtype_hvp_libs_libraries', array('library_id' => $libraryid));
    }
    ...
    public function deleteLibrary($library) {
        ...
        $DB->delete('qtype_hvp_libs_libraries', array('library_id' => $library->id));
        $DB->delete('qtype_hvp_libs_languages', array('library_id' => $library->id));
        $DB->delete('qtype_hvp_libraries', array('id' => $library->id));
    }
    ...
    public function getOption($name, $default = false) {
        $value = get_config('qtype_hvp', $name);
        if ($value === false) {
            return $default;
        }
        return $value;
    ...
    public function setOption($name, $value) {
        set_config($name, $value, 'qtype_hvp');
    }
    ...
    public function updateContentFields($id, $fields) {
        ...
        $DB->update_record('qtype_hvp', $content);
    }
    ...
    public function getNumContent($libraryid, $skip = NULL) {
        ...
        return (int) $DB->get_field_sql(
                "SELECT COUNT(id) FROM {qtype_hvp} WHERE main_library_id = ?{$skipquery}",
                array($libraryid));
    }
    ...
    public function isContentSlugAvailable($slug) {
        ...
        return !$DB->get_records_sql("SELECT id, slug FROM {qtype_hvp} WHERE slug = ?", array($slug));
    }
    ...
    public function saveCachedAssets($key, $libraries) {
        ...
        foreach ($libraries as $library) {
            ...
            $DB->insert_record('qtype_hvp_libs_cachedassets', $cachedasset);
        }
    }
    ...
    public function deleteCachedAssets($libraryid) {
        ...
        $results = $DB->get_records_sql(
                'SELECT hash
                   FROM {qtype_hvp_libs_cachedassets}
                  WHERE library_id = ?',
                array($libraryid));
        ...
        foreach ($results as $key) {
            $hashes[] = $key->hash;
            $DB->delete_records('qtype_hvp_libs_cachedassets', array('hash' => $key->hash));
        }
    ...
    public function hasPermission($permission, $cmid = null) {
        switch ($permission) {
            ...
            case H5PPermission::COPY_H5P:
                ...
                return has_capability('qtype/hvp:getexport', $cmcontext);
            case H5PPermission::CREATE_RESTRICTED:
                return has_capability('qtype/hvp:userestrictedlibraries', $this->getajaxcoursecontext());
            case H5PPermission::UPDATE_LIBRARIES:
                $context = \context_system::instance();
                return has_capability('qtype/hvp:updatelibraries', $context);
            case H5PPermission::INSTALL_RECOMMENDED:
                return has_capability('qtype/hvp:installrecommendedh5plibraries', $this->getajaxcoursecontext());
            case H5PPermission::EMBED_H5P:
                $cmcontext = \context_module::instance($cmid);
                return has_capability('qtype/hvp:getembedcode', $cmcontext);
        }
        return false;
    }
    ...
    public function replaceContentTypeCache($contenttypecache) {
        ...
        $DB->delete_records('qtype_hvp_libs_hub_cache');
        foreach ($contenttypecache->contentTypes as $ct) {
            $DB->insert_record('qtype_hvp_libs_hub_cache', (object) array(
                'machine_name'      => $ct->id,
    ...
    public function replaceContentHubMetadataCache($metadata, $lang = 'en') {
        ...
        $cache = $DB->get_record_sql(
            'SELECT id
                   FROM {qtype_hvp_content_hub_cache}
                  WHERE language = ?',
            array($lang)
        );
        if ($cache) {
            // Update.
            $DB->execute("UPDATE {qtype_hvp_content_hub_cache} SET json = ? WHERE id = ?", array($metadata, $cache->id));
        } else {
            // Insert.
            $DB->insert_record('qtype_hvp_content_hub_cache', (object) array(
                'json'         => $metadata,
                'language'     => $lang,
                'last_checked' => time(),
            ));
    ...
    public function setContentHubMetadataChecked($time, $lang = 'en') {
        global $DB;
        $DB->execute("UPDATE {qtype_hvp_content_hub_cache} SET last_checked = ? WHERE language = ?", array($time, $lang));
    }
}
