 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_diagnostic_base extends question_graded_automatically {
    ...
    public static $certainties = array(self::LOW, self::MED);
    public $answers;

    public $shuffleanswers;
    public $answernumbering;
    public $otheranswer;
    public $otheranswertext;
    public $answerreason;
    public $addcbm;
	...
	//public $certainties = array('בטוחים', 'לא בטוחים');
    ...
    public function apply_attempt_state(question_attempt_step $step) {
        ...
        foreach ($this->order as $ansid) {
            ...
            $a = new stdClass();
            $a->id = 0;
            $a->answer = html_writer::span(get_string('deletedchoice', 'qtype_diagnostic'),
                    'notifyproblem');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_diagnostic_single_question extends qtype_diagnostic_base {
    public function get_renderer(moodle_page $page) {
        return $page->get_renderer('qtype_diagnostic', 'single');
    }
    ...
    public function get_expected_data() {
        $expected = array();
        $expected['answer'] = PARAM_INT;
        $expected['other'] = PARAM_BOOL;
        $expected['other_data'] = PARAM_RAW_TRIMMED;
        $expected['reason'] = PARAM_RAW_TRIMMED;
        $expected['cbm'] = PARAM_INT;
        $expected['cbmnotsure'] = PARAM_RAW_TRIMMED;
    ...
    public function is_complete_response(array $response) {
        $is_answer = $is_reason = $is_cbm = false;

		file_put_contents('/tmp/aaa.php', print_r($response, 1));
        if(array_key_exists('answer', $response) && $response['answer'] !== '' && $response['answer'] !== -1) {
            $is_answer = true;
        } else if(array_key_exists('other', $response) && $response['other'] !== '' && $response['other_data'] !== '') {
            $is_answer = true;
        }

        if($this->addcbm) {
            if(array_key_exists('cbm', $response) && $response['cbm'] !== '') {
                $is_cbm = true;
            }
        } else {
            $is_cbm = true;
        ...
        if($this->answerreason) {
            if(array_key_exists('reason', $response) && $response['reason'] !== '') {
                $is_reason = true;
            }
        } else {
            $is_reason = true;
        ...
        return $is_answer && $is_cbm && $is_reason;
    }
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseselectananswer', 'qtype_diagnostic');
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_diagnostic_multi_question extends qtype_diagnostic_base {
    public function get_renderer(moodle_page $page) {
        return $page->get_renderer('qtype_diagnostic', 'multi');
    }
    ...
    public function is_complete_response(array $response) {
        ...
        if(!$is_answer) {
            if(array_key_exists('other', $response) && $response['other'] !== '' && $response['other_data'] !== '') {
                $is_answer = true;
            }
        }
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseselectatleastoneanswer', 'qtype_diagnostic');
    }
