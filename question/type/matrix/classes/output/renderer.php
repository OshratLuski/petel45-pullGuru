...
class renderer extends qtype_with_combined_feedback_renderer {
        ...
        foreach ($order as $rowid) {
            ...
            foreach ($question->cols as $col) {
                ...
                if ($question->multiple) {
                    $cell = self::checkbox($cellname, $ischecked, $isreadonly, $feedback);
                } else {
                    $cell = self::radio($cellname, $col->id, $ischecked, $isreadonly, $feedback);
                }
                //$weight = $question->weight($row, $col);
    ...
    protected static function checkbox(string $name, bool $checked, bool $readonly, $feedback): string {
    ...
    protected static function radio(string $name, string $value, bool $checked, bool $readonly, $feedback): string {
        $readonly = $readonly ? 'readonly="readonly" disabled="disabled"' : '';
        $checked = $checked ? 'checked="checked"' : '';
        ...
        if($feedback){
            return '
                <div style="display: inline-flex;
                    ...
                    align-items: center;
                    justify-content: center;">
                    <input type="radio" name="'.$name.'" style="margin: 0" '.$checked.' '.$readonly.' />
                </div>
            ';
        }
