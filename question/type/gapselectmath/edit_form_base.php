 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_gapselectmath_edit_form_base extends question_edit_form {
    ...
    private function allowed_tags_message($badtag) {
        ...
        if ($a->allowed) {
            return get_string('tagsnotallowed', 'qtype_gapselectmath', $a);
        } else {
            return get_string('tagsnotallowedatall', 'qtype_gapselectmath', $a);
        }
    }
    ...
    protected function definition_answer_choice(&$mform) {
        $mform->addElement('header', 'choicehdr', get_string('choices', 'qtype_gapselectmath'));
        $mform->setExpanded('choicehdr', 1);

        $mform->addElement('checkbox', 'shuffleanswers', get_string('shuffle', 'qtype_gapselectmath'));
        $mform->setDefault('shuffleanswers', 0);

        $mform->addElement('static', 'firstiscorrect', get_string('firstiscorrect', 'qtype_gapselectmath'));

        $textboxgroup = array();
        $textboxgroup[] = $mform->createElement('group', 'choices',
                get_string('choicex', 'qtype_gapselectmath'), $this->choice_group($mform));
        ...
        $this->repeat_elements($textboxgroup, $repeatsatstart, $repeatedoptions,
                ...
                get_string('addmorechoiceblanks', 'qtype_gapselectmath'), true);
    }
    ...
    protected function choice_group($mform) {
        ...
        $grouparray = array();
        $grouparray[] = $mform->createElement('mathlive', 'answer',
                get_string('answer', 'qtype_gapselectmath'), array('size' => 30, 'class' => 'tweakcss'));
        $grouparray[] = $mform->createElement('select', 'choicegroup',
                get_string('group', 'qtype_gapselectmath'), $options);
        return $grouparray;
    }
    ...
    public function validation($fromform, $files) {
        ...
        $questiontext = $fromform['questiontext'];
        $choices = $fromform['choices'];
    ...
    private function validate_slots($questiontext, $choices) {
        ...
        if (!$questiontext) {
            return get_string('errorquestiontextblank', 'qtype_gapselectmath');
        }
        ...
        if (!$slots) {
            return get_string('errornoslots', 'qtype_gapselectmath');
        }
        ...
        foreach ($slots as $slot) {
            ...
            foreach ($choices as $key => $choice) {
                if ($slot == $key + 1) {
                    if ($choice['answer'] === '') {
                        return get_string('errorblankchoice', 'qtype_gapselectmath',
            ...
            if (!$found) {
                return get_string('errormissingchoice', 'qtype_gapselectmath',
                        html_writer::tag('b', $slot));
            }
        }
        return false;
