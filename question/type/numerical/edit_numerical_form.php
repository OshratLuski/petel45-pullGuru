...
class qtype_numerical_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        ...
        $numericalunits = qtype_numerical\autocomplete::get_units_array();
    ...
    protected function add_unit_options($mform) {
        ...
        $unitinputoptions = array(
            ...
            qtype_numerical::UNITAUTOCOMPLETE  => get_string('unitnewtype', 'qtype_numerical'),
        );
        ...
        $mform->setDefault('multichoicedisplay',
                $this->get_default_value('multichoicedisplay', qtype_numerical::UNITAUTOCOMPLETE));
    ...
    protected function validate_numerical_options($data, $errors) {
        ...
        if($data['multichoicedisplay'] == 3 && $data['unitrole'] == 1){

            // Check answer.
            $errorkeys = [];

            foreach ($data['answer'] as $key => $answerval) {
                $erroranswer = false;
                $errorunit = false;

                // Answer value.
                if (strlen($answerval) == 0 || !is_numeric($answerval)) {
                    $erroranswer = true;
                }

                // Unit value.
                if (empty($data['unitvalue'][$key])) {
                    $errorunit = true;
                }

                if (($erroranswer && !$errorunit) || (!$erroranswer && $errorunit)) {
                    $errorkeys[] = $key;
                }
            }

            foreach (array_unique($errorkeys) as $key) {
                $errors['answeroptions['.$key.']'] = get_string('requiredvalueandunit', 'qtype_numerical');
            }
