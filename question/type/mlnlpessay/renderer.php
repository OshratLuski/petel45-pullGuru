...
class qtype_mlnlpessay_renderer extends qtype_renderer {
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if (!empty($qa->get_question()->filetypeslist)) {
            $text = html_writer::tag('p', get_string('acceptedfiletypes', 'qtype_mlnlpessay'));
    ...
    public function specific_feedback(question_attempt $qa, $withjs = true) {
        global $DB, $PAGE, $CFG;
        ...
        $data['qaid'] = $qa->get_database_id();
        $data['selector'] = 'mlnlpresponse';
        $truefeedback = '';

        if ($pythonfeedbacksql == null) {
            if ($withjs) {
                $waithtml = $this->output->render_from_template('qtype_mlnlpessay/wait', $data);
                $truefeedback = $waithtml;

                $PAGE->requires->js_call_amd('qtype_mlnlpessay/feedback', 'init', $data);
                $PAGE->requires->js_call_amd('qtype_mlnlpessay/hide', 'init', $data);
            }
        } else {

            $truefeedback = get_response($pythonfeedbacksql, $data);
            if ($withjs) {
                $PAGE->requires->js_call_amd('qtype_mlnlpessay/hide', 'init', $data);
                $PAGE->requires->js_call_amd('qtype_mlnlpessay/override', 'init',
                        ['mlnlpessay-container', $data['qid'] . $pythonfeedbacksql->questionattemptid]);
            }
        }
        $question->truefeedback = $truefeedback;
        $question->trueanswerid = 1;

        $restoreval = $CFG->forceclean;
        //We shut down forceclean parameter to keep checkboxes
        $CFG->forceclean = false;

        $return = $question->format_text($question->truefeedback, $question->generalfeedbackformat,
            $qa, 'question', 'answerfeedback', $question->trueanswerid);

        $CFG->forceclean = $restoreval;
...
/**
 * An mlnlpessay format renderer for essays where the student should not enter
 * any inline response.
 *
 * @copyright  2013 Binghamton University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mlnlpessay_format_noinline_renderer extends plugin_renderer_base {

    protected function class_name() {
        return 'qtype_mlnlpessay_noinline';
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mlnlpessay_format_editorfilepicker_renderer extends qtype_mlnlpessay_format_editor_renderer {
    protected function class_name() {
        return 'qtype_mlnlpessay_editorfilepicker';
    }
    ...
    protected function specific_filepicker_options($acceptedtypes, $draftitemid, $context) {
        debugging('qtype_mlnlpessay_format_editorfilepicker_renderer::specific_filepicker_options() is deprecated, ' .
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mlnlpessay_format_plain_renderer extends plugin_renderer_base {
    /** @var question_display_options Question display options instance for any necessary information for rendering the question. */
    ...
    protected function textarea($response, $lines, $attributes) {
        $attributes['class'] = $this->class_name() . ' qtype_mlnlpessay_response';
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mlnlpessay_format_monospaced_renderer extends qtype_mlnlpessay_format_plain_renderer {
    protected function class_name() {
        return 'qtype_mlnlpessay_monospaced';
    }
}
