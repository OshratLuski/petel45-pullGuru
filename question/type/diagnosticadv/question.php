...
class qtype_diagnosticadv_question extends question_graded_by_strategy
    ...
    public function summarise_response(array $response) {
        if (isset($response['customanswer']) && $response['answer'] === 'custom') {
            return $response['customanswer'];
        } elseif (isset($response['answer'])) {
            $ansid = $response['answer'];
            if (isset($this->answers[$ansid])) {
                $answer = $this->answers[$ansid];
                return format_text($answer->answer, $answer->answerformat, ['context' => $this->context]);
            }
            return $ansid;
        }
        return null;
    ...
    public function get_validation_error(array $response) {
        ...
        $answercheck = array_key_exists('answer', $response) && !empty($response['answer']);
        $customanswercheck = array_key_exists('customanswer', $response) && ($response['customanswer'] || $response['customanswer'] === '0');
        $hasanswer = $answercheck || $customanswercheck;

        //if (!$hasanswer) {
        //    $errors['answer'] = get_string('pleaseenterananswer', 'qtype_diagnosticadv');
        //}

        if ($this->required) {
            if (!isset($response['comment']) || empty($response['comment'])) {
                $errors['comment'] = get_string('pleaseentercomment', 'qtype_diagnosticadv');
            }
        }
    ...
    public function check_file_access($qa, $options, $component, $filearea, $args, $forcedownload) {
        if ($component == 'question' && $filearea == 'answer') {
            $answerid = reset($args);
            return isset($this->answers[$answerid]);
