...
/**
 * Opensheet question renderer class.
 *
 * @package    qtype
...
/**
 * Generates the output for opensheet questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_opensheet_renderer extends qtype_renderer {

    public function head_code(question_attempt $qa) {

        global $PAGE;

        $PAGE->requires->css('/question/type/opensheet/jquery.sheet/jquery-ui/theme/jquery-ui.min.css');
        $PAGE->requires->css('/question/type/opensheet/jquery.sheet/plugins/jquery.colorPicker.css');

        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/jquery-1.10.2.min.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/jquery-ui/ui/jquery-ui.min.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/jquery.sheet.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/globalize.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/parser/formula/formula.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/parser/tsv/tsv.js'), true);

        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/jquery.nearest.min.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/raphael-min.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/g.raphael-min.js'), true);

        //$PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/jquery.colorPicker.min.js'), true);

        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/jquery.elastic.min.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/jquery.sheet.advancedfn.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/jquery.sheet.financefn.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/undomanager.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/ZeroClipboard.js'), true);
        $PAGE->requires->js(new moodle_url('/question/type/opensheet/jquery.sheet/plugins/jquery.sheet.dts.js'), true);
    ...
    public function build_sheet(question_attempt $qa, question_display_options $options) {
        global $CFG, $PAGE;
        ...
        if(!empty($response)) {
            ...
            foreach($data as $key => $sheet){
                if(!isset($sheet->rows) || empty($sheet->rows)){
                    unset($data[$key]);
                }
            }
        ...
        if(empty($response)) {
            ...
            // Empty table.
            $tablehtml = "<table title='Sheet 1'>";

            for($j = 0; $j < $maxcountrows; $j++) {

                $htmlrow = '';
                for ($i = 0; $i < $maxcountcolumns; $i++) {
                    $htmlrow .= '<td></td>';
                }

                $tablehtml .= '<tr>' . $htmlrow . '</tr>';
            }
            ...
            foreach($data as $key => $sheet){
                if(isset($sheet->rows)){
                    foreach($sheet->rows as $key2 => $row){

                        $delta = $maxcountcolumns - count($row->columns);
                        if($delta > 0){
                            for($i = 0; $i < $delta; $i++){
                                $data[$key]->rows[$key2]->columns[] = new \StdClass();
                            }
                        }
                    }
                }
            ...
            foreach($data as $sheet){
                if(isset($sheet->rows) && isset($sheet->rows[0])){
                    $objrowempty = clone $sheet->rows[0];
                    foreach($objrowempty->columns as $key => $item){
                        $objrowempty->columns[$key] = new \StdClass();
                    }
            ...
            if($objrowempty != null){
                foreach($data as $key => $sheet){
                    if(isset($sheet->rows)){
                        ...
                        $delta = $maxcountrows - count($sheet->rows);
                        if($delta > 0){
                            for($i = 0; $i < $delta; $i++){
                                $data[$key]->rows[] = $objrowempty;
                            }
                        }
                    }
                }
        ...
        $js = "
            <script type=\"text/javascript\">
                ...
                (function(jQuery){            
                    $.sheet.preLoad('".$CFG->wwwroot ."/question/type/opensheet/jquery.sheet/');
                    ...
                    $(function() {
                        ...
                        var tempUrl = '".$CFG->wwwroot."/question/type/opensheet/jquery.sheet/menu.right.html';
                    
                        $(document.createElement('div'))                                
                            .load(tempUrl, function() {
                                                            
                            var menurender = this.children[0]; 
                        ...
                        // Save calcsheet table.                                 
                        window.sheet = $.sheet;                        
                        $('form').submit(function(e){
                                                    
                          var jS = obj.getSheet();
                          //var jS = $.sheet.instance[0];
                          //var jS = window.sheet.instance[0];                          
                          //var json = $.sheet.dts.fromTables.json(jS);
                          
                          var dataSheet = window.sheet.dts.fromTables.json(jS); 
                          var typeChart = $('#".$idtable."_typeChart').val();
                          var dataChart = $('#".$idtable."_dataChart').val();
                          var titleChart = $('#".$idtable."_titleChart').val();
                          
                          var data = {'dataSheet': dataSheet, 'typeChart': typeChart, 'dataChart': dataChart, 'titleChart': titleChart};
                                                                             
                          $('#".$idinput."').val(JSON.stringify(data));                          
                        });
                    });
                })($);
        ...
        $html = $js;
        $html .= '<input type="hidden" id="'.$idinput.'" name="'.$qa->get_qt_field_name('answer').'" value="'.$response.'" />';

        $html .= "
            <div id='".$idtable."' title=''  style='min-height: 315px; width: 780px;' >".$tablehtml."</div>                    
        ";

        // Chart JS.
        $resp->typeChart = isset($resp->typeChart) ? $resp->typeChart : "";
        $resp->dataChart = isset($resp->dataChart) ? $resp->dataChart : "";

        $resp->refreshChart = 0;
        if(!empty($resp->typeChart) && !empty($resp->dataChart)){
            $resp->refreshChart = 1;
        }
        ...
        <div style="width: 780px;"> 
            <div class="control-chart pt-3">
                <div class="form-group row">
                    <div class="col-sm-2">
                        <label for="'.$idtable.'_titleChart" class="d-flex align-items-center mb-0 mr-2">'.get_string('charttitle', 'qtype_opensheet').'</label>
                    </div>     
                    <div class="col-sm-10">
                        <input '.$disabled.' id="'.$idtable.'_titleChart" class="form-control" type="text" value="'.$resp->titleChart.'" style="min-width: 40%">  
                    </div>          
                </div>

                <div class="form-group row align-items-center">
                    <div class="col-sm-2">
                        <label for ="'.$idtable.'_selectChart" class="mb-0 mr-2">'.get_string('charttype', 'qtype_opensheet').'</label>
                    </div>
                    <div class="col-sm-10">
                        <select '.$disabled.' id="'.$idtable.'_selectChart" class="form-control"  style="min-width: 40%">                            
                            <option value="SCATTERCHART">'.get_string('scatterchart', 'qtype_opensheet').'</option>
                            <option value="BARCHART">'.get_string('barchart', 'qtype_opensheet').'</option>
                            <option value="PIECHART">'.get_string('piechart', 'qtype_opensheet').'</option>
        ...
        $html .= '
            <div class="chart-image justify-content-center" id="'.$idtable.'_defaultImage" style="min-height: 315px; width: 780px; display: flex;">            
                <img src="'.$CFG->wwwroot.'/question/type/opensheet/img/default.png" alt="Default image">                                
            </div>
        ';

        $html .= '
            <div class="chart-js" style="display:none; min-height: 315px; width: 780px;">            
                <canvas id="'.$idtable.'_externalChart" style="width:100%;max-width:800px"></canvas>
                                
                <input type="hidden" id="'.$idtable.'_typeChart" value="'.$resp->typeChart.'"/>
                <input type="hidden" id="'.$idtable.'_dataChart" value='."'".$resp->dataChart."'".'/>
                <input type="hidden" id="'.$idtable.'_refreshChart" value="'.$resp->refreshChart.'"/>                                
            </div>
        ';

        // Add chart js.
        $PAGE->requires->js_call_amd('qtype_opensheet/chart', 'init', [$idtable]);
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        $result .= $this->build_sheet($qa, $options);
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if (!empty($qa->get_question()->filetypeslist)) {
            $text = html_writer::tag('p', get_string('acceptedfiletypes', 'qtype_opensheet'));
    ...
    public function manual_comment(question_attempt $qa, question_display_options $options) {
        ...
        return html_writer::nonempty_tag('div', $question->format_text(
                $question->graderinfo, $question->graderinfo, $qa, 'qtype_opensheet',
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_opensheet_format_renderer_base extends plugin_renderer_base {
    /**
...
/**
 * An opensheet format renderer for opensheets where the student should not enter
 * any inline response.
 *
 * @copyright  2013 Binghamton University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_opensheet_format_noinline_renderer extends plugin_renderer_base {

    protected function class_name() {
        return 'qtype_opensheet_noinline';
    }
...
/**
 * An opensheet format renderer for opensheets where the student should use the HTML
 * editor without the file picker.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_opensheet_format_editor_renderer extends plugin_renderer_base {
    protected function class_name() {
        return 'qtype_opensheet_editor';
    }

    public function response_area_read_only($name, $qa, $step, $lines, $context) {
        return html_writer::tag('div', $this->prepare_response($name, $qa, $step, $context),
                ['class' => $this->class_name() . ' qtype_opensheet_response readonly',
    ...
    public function response_area_input($name, $qa, $step, $lines, $context) {
        ...
        $output .= html_writer::start_tag('div', array('class' =>
                $this->class_name() . ' qtype_opensheet_response'));
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_opensheet_format_editorfilepicker_renderer extends qtype_opensheet_format_editor_renderer {
    protected function class_name() {
        return 'qtype_opensheet_editorfilepicker';
    }
    ...
    protected function specific_filepicker_options($acceptedtypes, $draftitemid, $context) {
        debugging('qtype_opensheet_format_editorfilepicker_renderer::specific_filepicker_options() is deprecated, ' .
...
/**
 * An opensheet format renderer for opensheets where the student should use a plain
 * input box, but with a normal, proportional font.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_opensheet_format_plain_renderer extends plugin_renderer_base {
    /**
     * @return string the HTML for the textarea.
     */
    protected function textarea($response, $lines, $attributes) {
        $attributes['class'] = $this->class_name() . ' qtype_opensheet_response';
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_opensheet_format_monospaced_renderer extends qtype_opensheet_format_plain_renderer {
    protected function class_name() {
        return 'qtype_opensheet_monospaced';
    }
}
