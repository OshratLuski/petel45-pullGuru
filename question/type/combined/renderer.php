...
class qtype_combined_renderer extends qtype_with_combined_feedback_renderer {
    ...
    public function feedback(question_attempt $qa, question_display_options $options) {
        ...
        if ($options->rightanswer) {
            $rightsanswers = $qa->get_question()->combiner->call_all_subqs('get_right_answer_summary');
            if (count($rightsanswers) == 1) {
                $rightanswertext = format_text($rightsanswers[0], FORMAT_HTML);
                $output .= html_writer::nonempty_tag('div',
                        get_string('correctansweris', 'qtype_combined', $rightanswertext), array('class' => 'specificfeedback'));
            }
        }
