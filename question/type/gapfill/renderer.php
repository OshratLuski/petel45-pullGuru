...
class qtype_gapfill_renderer extends qtype_with_combined_feedback_renderer {
    ...
    public function embedded_element(question_attempt $qa, $place, question_display_options $options, $markedgaps) {
        ...
        if ($question->answerdisplay == "dropdown") {
            ...
            if(isset($inputattributes['disabled']) && $inputattributes['disabled']) {
                ...
                if (strpos($question->questiontext, '<table') == false || strpos($question->questiontext, '</table>') == false) {
                    return $this->custom_drag_place($currentanswer, $inputattributes, $aftergaptext);
                }
                ...
                if ($currentanswer) {
                    ...
                    foreach ($this->allanswers as $potentialanswer) {
                        ...
                        // PTL-11912.
                        $potentialanswer = trim($potentialanswer);
                ...
                if (!empty($formula)) {
                    $classes = ' position-relative mr-2 dropped';
    ...
    private function custom_drag_place($currentanswer, $inputattributes, $aftergaptext) {
        ...
        $html = html_writer::start_tag('span', array('class' => 'droptarget-wrapper d-inline-flex position-relative align-items-center'));
        $html .= html_writer::start_tag('span', array('class' => 'droptarget d-inline-block px-2 h-100 bg-white rounded border ' . $classes,
                'style' => $styles));
