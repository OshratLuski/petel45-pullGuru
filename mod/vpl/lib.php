...
function vpl_extend_settings_navigation(settings_navigation $settings, navigation_node $vplnode) {
    ...
    if ($manager) {
        ...
        $node = vpl_navi_node_create($vplnode, 'testactivity', $url, navigation_node::TYPE_SETTING);
        $vplnode->add_node( $node, $fkn );
        $url = new moodle_url( '/mod/vpl/views/submissionslist.php', $parms );
        $node = vpl_navi_node_create($vplnode, 'submissionslist', $url, navigation_node::TYPE_SETTING);
        $vplnode->add_node( $node, $fkn );
        $url = new moodle_url( '/mod/vpl/forms/executionfiles.php', $parms );
        $node = vpl_navi_node_create($vplnode, 'vplcomponents', $url, navigation_node::TYPE_SETTING);
        $vplnode->add_node( $node, $fkn );
