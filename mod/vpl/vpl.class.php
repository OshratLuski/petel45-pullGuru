...
class mod_vpl {
    ...
    public function print_view_tabs($path) {
        ...
        // Generate main tabs if user has higher-level capabilities
        if ($level2) {
            $maintabs = $this->get_main_tabs($cmid, $userid, $USER, $viewer, $grader, $manager, $active);
        }

        // Generate tabs based on the active page
        switch ($active) {
            case 'view.php':
                if ($level2) {
                    $this->display_tabs($maintabs, $tabs, $active);
                    return;
                }
            ...
            case 'previoussubmissionslist.php':
                $tabs = $this->get_submission_tabs($cmid, $userid, $USER, $viewer, $submiter, $grader, $manager, $example);
                $this->display_user_picture($userid);
                $this->display_tabs($maintabs, $tabs, $active);
                return;
            case 'submissionslist.php':
                $this->display_tabs($maintabs, [], $active);
                return;
            case 'listwatermark.php':
            case 'similarity_form.php':
            case 'listsimilarity.php':
                if ($similarity) {
                    $tabs = $this->get_similarity_tabs($cmid, $active);
                }
            // No break to fall through
            ...
            case 'executionkeepfiles.php':
                $this->display_tabs($maintabs, $tabs, $active);
                break;
        }
    }
    /**
     * Generates the main navigation tabs based on user capabilities.
     *
     * @param int    $cmid    The course module ID.
     * @param int    $userid  The user ID.
     * @param object $USER    The global user object.
     * @param bool   $viewer  Whether the user has view capability.
     * @param bool   $grader  Whether the user has grade capability.
     * @param bool   $manager Whether the user has manage capability.
     * @param string $active  The active tab name.
     *
     * @return array The array of main tab objects.
     */
    private function get_main_tabs($cmid, $userid, $USER, $viewer, $grader, $manager, $active) {
        $vplactivity = ['view.php'];
        $submissionlist = ['submissionlist.php'];
        $vplcomponents = ['executionfiles.php', 'testcasesfile.php', 'requiredfiles.php', 'executionoptions.php', 'executionkeepfiles.php'];
        $maintabs = [];

        if(in_array($active , $vplactivity)) {
        ...
        if(in_array($active , $submissionlist)) {
            // Add submissions list tab
        ...
        if(in_array($active , $vplcomponents)) {
            // Add execution files tab
            ...
            // Add execution keep files tab
            $href = vpl_mod_href('forms/executionkeepfiles.php', 'id', $cmid);
            $maintabs[] = vpl_create_tabobject('executionkeepfiles.php', $href, 'keepfiles');
        }
    ...
    /**
     * Generates the submission-related tabs based on user capabilities and activity state.
     *
     * @param int    $cmid     The course module ID.
     * @param int    $userid   The user ID.
     * @param object $USER     The global user object.
     * @param bool   $viewer   Whether the user has view capability.
     * @param bool   $submiter Whether the user has submit capability.
     * @param bool   $grader   Whether the user has grade capability.
     * @param bool   $manager  Whether the user has manage capability.
     * @param bool   $example  Whether the activity is marked as an example.
     *
     * @return array The array of submission tab objects.
     */
    private function get_submission_tabs($cmid, $userid, $USER, $viewer, $submiter, $grader, $manager, $example) {
        require_once('vpl_submission.class.php');
        $subinstance = $this->last_user_submission($userid);
        $tabs = [];

        // Add view tab if viewer but not higher-level capabilities
        if ($viewer && !($grader || $manager || $this->has_capability(VPL_SIMILARITY_CAPABILITY))) {
            $href = vpl_mod_href('view.php', 'id', $cmid, 'userid', $userid);
            $tabs[] = vpl_create_tabobject('view.php', $href, 'description');
        }
        ...
        if ($manager || ($grader && $USER->id == $userid) || (!$grader && $submiter && $this->is_submit_able())) {
            ...
            $stredit = ($example && $this->instance->run) ? 'run' : 'edit';
            $tabs[] = vpl_create_tabobject('edit.php', $href, $stredit);
        }

        // Add submission tab
        if ($manager || ($grader && $USER->id == $userid) || (!$grader && $submiter && $this->is_submit_able() && !$this->instance->restrictededitor && !$example)) {
            $href = vpl_mod_href('forms/submission.php', 'id', $cmid, 'userid', $userid);
            $tabs[] = vpl_create_tabobject('submission.php', $href, 'submission');
        }

        // Add submission view and grading tabs
        if (!$example) {
            ...
            if ($grader && $this->get_grade() != 0 && $subinstance && ($subinstance->dategraded == 0 || $subinstance->grader == $USER->id || $subinstance->grader == 0)) {
            ...
            // Add previous submissions tab
            if ($subinstance && ($grader || $this->has_capability(VPL_SIMILARITY_CAPABILITY))) {
    ...
    /**
     * Generates the similarity-related tabs if the user has similarity capability.
     *
     * @param int    $cmid   The course module ID.
     * @param string $active The active tab name.
     *
     * @return array The array of similarity tab objects.
     */
    private function get_similarity_tabs($cmid, $active) {
        $tabs = [];
    ...
    private function display_user_picture($userid) {
    ...
    /**
     * Displays the navigation tabs using Moodle's tab printing function.
     *
     * @param array  $maintabs The main tabs array.
     * @param array  $tabs     The sub-tabs array.
     * @param string $active   The active tab name.
     *
     * @return void
     */

    private function display_tabs($maintabs, $tabs, $active) {
        if (!empty($maintabs)) {
    ...
    public function str_submission_restriction($userid = null) {
        ...
        if (count( $files )) {
            ...
            $link .= '</a>)';
            $html .= $this->str_restriction_with_icon( 'requestedfilescustom', $text . $link );
        }
        $instance = $this->get_instance();
