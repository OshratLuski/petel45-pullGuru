...
class mod_quiz_mod_form extends moodleform_mod {
    ...
    protected function definition() {
        ...
        // PTL-7972. Enable checkboxes "closed".
    ...
    public function add_completiongrade_rules(): array {
        ...
        $completionpassel = 'completionpass' . $suffix;

        $group = array();
        $group[] = $mform->createElement('advcheckbox', $completionpassel, null, get_string('completionpasscustom', 'quiz'),
                array('group' => 'cpass'));
        $mform->disabledIf($completionpassel, 'completionusegrade', 'notchecked');

        $group[] = $mform->createElement('advcheckbox', $completionattemptsexhaustedel, null,
                get_string('completionattemptsexhausted', 'quiz'),
        ...
        $mform->disabledIf($completionattemptsexhaustedel, $completionpassel, 'notchecked');
        $mform->addGroup($group, 'completionpassgroup', '', ' &nbsp; ', false);

        return [$completionpassel, $completionattemptsexhaustedel];
    }
