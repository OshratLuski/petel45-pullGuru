...
if (data_submitted() && confirm_sesskey()) {
    if (optional_param('submit', false, PARAM_BOOL) && question_engine::is_manual_grade_in_range($attemptobj->get_uniqueid(), $slot)) {
        ...
        $prefix = 'q' . $attemptobj->get_uniqueid() . ':' . $slot . '_';
        ...
        $comment = optional_param($prefix . '-comment', '', PARAM_RAW);
...
$PAGE->requires->js_amd_inline("require(['jquery', 'core/str'], function($, Str) {
    $('.info').removeClass('d-flex');
    
    Str.get_strings([
        { key: 'saveandupdatethestudent', component: 'quiz_assessmentdiscussion' }                    
    ]).done(function (strings) {
        $('#id_submitbutton').val(strings[0]);                
    })
    
     $('#manualgradingform').on('submit', function(e){
       
        const gradeInput = document.getElementById('".$prefix."-mark');
        const grade = parseFloat(gradeInput.value);
                 
        const maxmarkInput = document.getElementsByName('".$prefix."-maxmark');
        const maxmark = parseFloat(maxmarkInput[0].value);
              
        if (isNaN(grade) || grade > maxmark || grade < 0) {            
            e.preventDefault();
        }
...
echo '<form method="post" class="mform" id="manualgradingform" action="' .
        $CFG->wwwroot . '/mod/quiz/report/assessmentdiscussion/iframecomment.php">';
