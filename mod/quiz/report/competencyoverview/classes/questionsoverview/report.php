...
/**
 * This file defines the quiz questionsoverview report class.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_questionsoverview_report extends mod_quiz\local\reports\attempts_report {
    ...
    public function get_html_table($compid, $quiz, $cm, $course, $qset) {
        ...
        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
            'questionsoverview',
            'quiz_questionsoverview_settings_form',
            $quiz,
            $cm,
            $course
        );
        ...
        $options = new quiz_questionsoverview_options('teacheroverview', $quiz, $cm, $course);
        ...
        // Get only set of question. $qset.
        $questions = quiz_questionsoverview_report::quiz_report_get_significant_questions_set($quiz, $qset);
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            $html .= $table->htmlout($options->pagesize, true);
        }
    ...
    public function display($quiz, $cm, $course) {
        ...
        $options = new quiz_questionsoverview_options('questionsoverview', $quiz, $cm, $course);
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                echo '<a href="' . $urlbutton . '" class="btn btn-secondary">' . $namebutton . '</a>';
                echo '</div>';
                echo '</br>';
            }
            ...
            if (
                ...
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'quiz_questionsoverview');
            }
        ...
        if (!$table->is_downloading()) {
            ...
            if (has_capability('mod/quiz:regrade', $this->context)) {
                ...
                if ($currentgroup) {
                    ...
                    $regradealllabel =
                        get_string('regradeallgroup', 'quiz_questionsoverview', $a);
                } else {
                    $regradealldrydolabel =
                        get_string('regradealldrydo', 'quiz_questionsoverview', $regradesneeded);
                    $regradealldrylabel =
                        get_string('regradealldry', 'quiz_questionsoverview');
                    $regradealllabel =
                        get_string('regradeall', 'quiz_questionsoverview');
                }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'quiz_questionsoverview'));
     ...
     */
    protected function regrade_attempts(
        $quiz,
        $dryrun = false,
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update(
                $a['done'],
                $a['count'],
                get_string('regradingattemptxofy', 'quiz_questionsoverview', $a)
            );
        }
    ...
    protected function close_attempts(
        $quiz,
        $cm,
        $dryrun = false,
        $groupstudents = array(),
    ...
    public static function get_bands_labels($bands, $bandwidth, $quiz) {
        ...
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = quiz_format_grade($quiz, ($i - 1) * $bandwidth * $coefficient) . ' - ' . quiz_format_grade($quiz, $i * $bandwidth * $coefficient);
        }
        return $bandlabels;
    }
    ...
    protected function add_user_columns($table, &$columns, &$headers) {
        ...
        if (!$table->is_downloading() && $CFG->grade_report_showuserimage && $this->displayfull) {
            $columns[] = 'picture';
            $headers[] = '';
        }
        if (!$table->is_downloading()) {
