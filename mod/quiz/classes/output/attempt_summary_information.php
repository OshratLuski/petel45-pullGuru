...
class attempt_summary_information implements renderable, named_templatable {
    ...
    public static function create_for_attempt(
        ...
        if (class_exists('\quiz_advancedoverview\quizdata')) {
            ...
            $nonanon = !$quizdata->get_anon_state_for_user($attemptobj->get_cm()->id);
        }

        // Prepare summary information about the whole attempt.
        if (!$attemptobj->get_quiz()->showuserpicture && $attemptobj->get_userid() != $USER->id && $nonanon) {
        ...
        $summary->add_item('startedon', get_string('startedon', 'quiz'), userdate($attempt->timestart, '%d %b %Y, %H:%M'));

        if ($attempt->state == quiz_attempt::FINISHED) {
            $summary->add_item('completedon', get_string('completedon', 'quiz'),
                userdate($attempt->timefinish, '%d %b %Y, %H:%M'));
