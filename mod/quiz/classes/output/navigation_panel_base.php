    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            ...
            if (!in_array($pagenum, $pagenums)) {
                ...
                $lqsoptions = \local_quiz_summary_option\funcs::get_quiz_config($this->attemptobj->get_cmid());

                if ($pages[$pagenum]['totalpointsinpage'] > 0 && !$lqsoptions->summary_grade) {
                    $a = new \stdClass();
                ...
                $buttons[] = new \mod_quiz\output\navigation_section_heading(format_string($heading) . $progresspage);
            }
