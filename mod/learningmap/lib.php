...
function learningmap_get_place_cm(cm_info $cm): array {
    ...
    foreach ($placestore->places as $p) {
        if (isset($p->linkedActivity)  && $p->linkedActivity != null && $p->selectedLinkType == 'linkedActivity') {
            $modules[] = $p->linkedActivity;
        } elseif (isset($p->externalLink) && $p->externalLink != null && $p->selectedLinkType == 'externalLink') {
            $modules[] = intval($p->externalLink);
        }
    }
    return $modules;
}
