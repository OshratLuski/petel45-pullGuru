...
class mapworker {
    ...
    public function process_map_objects(): void {
        ...
        foreach ($this->placestore['places'] as $place) {
            ...
            // Support for old maps: Handle maps created before 'selectedLinkType' was added.
            // Add a default value for 'selectedLinkType' if missing.
            if (!isset($place['selectedLinkType'])) {
                $place['selectedLinkType'] = 'linkedActivity';
            }

    
            // Remove places that are not linked to an activity or where the activity is missing.
            if ($place['selectedLinkType'] === 'linkedActivity' && !empty($place['linkedActivity']) && in_array($place['linkedActivity'], $allcms)) {
                ...
                $text = $placecm->get_formatted_name();
            } elseif ($place['selectedLinkType'] === 'externalLink' && !empty($place['externalLink'])) {
                // Add protocol if missing from external link.
                $textlink = $place['externalLink'];
                if (!preg_match('/^https?:\/\//', $textlink)) {
                    $textlink = 'https://' . $textlink;
                }
                $text = substr($textlink, 0, 50);
            ...
            if ($place['selectedLinkType'] === 'linkedActivity' && $this->activitymanager->is_completed($placecm)) {
                $completedplaces[] = $place['id'];
                $this->active[] = $place['id'];
            } elseif (!isset($placecm) && $place['selectedLinkType'] === 'externalLink') {
                $completedplacesFromSession = isset($SESSION->completedplaces) ? array_keys((array)$SESSION->completedplaces) : [];

                if (in_array($place['id'], $completedplacesFromSession) && !in_array($place['id'], $completedplaces)) {
                    $completedplaces[] = $place['id'];
                    $this->active[] = $place['id'];
                }
            }
            ...
            if (isset($placecm) && !$placecm->available) {
                $notavailable[] = $place['id'];
            }
            // Places that are not visible and not in stealth mode (i.e. reachable by link)
            // are impossible to reach.
            if (isset($placecm) && $placecm->visible == 0 && !$placecm->is_stealth()) {
                $impossible[] = $place['id'];
            }

            // External links are only shown on the map if they are marked as completed,
            // unless showall mode is active.
            if ($place['selectedLinkType'] === 'externalLink' && (empty($this->placestore['showall']) || $this->placestore['showall'] == 0)) {
                $placeIds = array_column($this->placestore['places'], 'id');
            
                if (!isset($completedplaces)) {
                    $completedplaces = [];
                }
            
                $completedplacesFromSession = isset($SESSION->completedplaces) ? array_keys((array)$SESSION->completedplaces) : [];
            
                $completedplaces = array_unique(array_merge($completedplaces ?? [], $completedplacesFromSession));
            
                if (in_array($place['id'], $this->placestore['startingplaces']) && !in_array($place['id'], $this->active)) {
                    $this->active[] = $place['id'];
                } elseif (in_array($place['id'], $completedplacesFromSession) && !in_array($place['id'], $completedplaces)) {
                    $completedplaces[] = $place['id'];
                    $this->active[] = $place['id'];
                } else {
                    $currentIndex = array_search($place['id'], $placeIds);
                    if ($currentIndex > 0 && in_array($placeIds[$currentIndex - 1], $completedplaces)) {
                        $this->active[] = $place['id'];
                        $impossibleKey = array_search($place['id'], $impossible);
                        if ($impossibleKey !== false) {
                            unset($impossible[$impossibleKey]);
                        }
            
                        $notAvailableKey = array_search($place['id'], $notavailable);
                        if ($notAvailableKey !== false) {
                            unset($notavailable[$notAvailableKey]);
                        }
                    } else {
                        if (!in_array($place['id'], $this->placestore['startingplaces'])) {
                            $notavailable[] = $place['id'];
