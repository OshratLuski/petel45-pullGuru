...
class get_learningmap extends external_api {
    ...
    public static function execute(int $cmid): array {
        ...
        // Retrieve learning map record and decode place data
        $map = $DB->get_record('learningmap', ['id' => $cminfo->instance], '*', MUST_EXIST);
        $placestore = json_decode($map->placestore);

        // Fetch completed places from session
        $completedPlacesMap = isset($SESSION->completedplaces) ? (array)$SESSION->completedplaces : [];

        // Filter external links to verify only completed places match the stored link
        $externalLinks = array_filter($placestore->places, function($place) {
            return $place->selectedLinkType === 'externalLink' && !empty($place->externalLink);
        });

        // Loop through each external link to check and remove non-matching links
        foreach ($externalLinks as $place) {
            if (isset($completedPlacesMap[$place->id]) && $completedPlacesMap[$place->id] !== $place->externalLink) {
                unset($completedPlacesMap[$place->id]);
            }
        }

        // Update the session with the refined completed places map
        $SESSION->completedplaces = (object)$completedPlacesMap;
