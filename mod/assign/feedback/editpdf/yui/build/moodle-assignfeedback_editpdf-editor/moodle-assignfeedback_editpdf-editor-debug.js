...
Y.extend(HTMLEDITOR, M.core.dialogue, {
    ...
    initializer: function(config) {
        ...
        document.querySelector('.yui3-htmleditor h5').innerText = M.util.get_string('htmleditor', 'assignfeedback_editpdf');
...
var HTMLCOMMENT = function(editor, gradeid, pageno, x, y, width, colour, rawtext) {
    ...
    this.draw = function(editnode) {
        ...
        drawable.nodes.forEach(element => {
            const htmlComment = element._node.querySelector(":scope > .htmlcomment");
            if (htmlComment) {
                let rawInner = htmlComment.innerHTML;
                rawInner = rawInner.replaceAll("«", "<")
                  .replaceAll("»", ">")
                  .replaceAll("¨", '"')
                  .replaceAll("§", "&");
                htmlComment.innerHTML = rawInner;
            };
        });
        return drawable;
    };
