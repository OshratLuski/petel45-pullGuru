...
$roleid = optional_param('roleid', 0, PARAM_INT);                   // 0 means all enrolled users.
...
foreach ($ejsapps as $ejsapp) {
    $ejsappnames[$ejsapp->id] = $multilang->filter($ejsapp->name);
    if ($i == 1 && $ejsappid == 0) {
        $ejsappid = $cm->instance;
    }
    $i++;
...
$rolenamesurl = new moodle_url($CFG->wwwroot .
    "/mod/ejsapp/personalized_vars_values.php?id=$id&courseid=$courseid&sifirst=&silast=");
...
if ($personalized) { // If there is at least one ejsapp activity with personalized variables.
    ...
    $exportfilename = get_string('personalized_values', 'ejsapp').$ejsappnames[$ejsapp->id];
    ...
    if ($mycourses = enrol_get_my_courses()) {
        $courselist = array();
        $popupurl = new moodle_url('/mod/ejsapp/personalized_vars_values.php?id='.$id.'&courseid='.$courseid.
            '&roleid='.$roleid.'&sifirst=&silast=');
        ...
        $select = new single_select($popupurl, 'id', $courselist, $course->id,
    ...
    if (!isset($hiddenfields['lastaccess'])) {
        ...
        // We need to make it differently for normal courses and site course.
        if (!$isfrontpage) {
            $minlastaccess = $DB->get_field_sql('SELECT min(timeaccess)
                                                 FROM {user_lastaccess}
                                                 WHERE courseid = ?
                                                 AND timeaccess != 0', array($course->id));
            $lastaccess0exists = $DB->record_exists('user_lastaccess',
                array('courseid' => $course->id, 'timeaccess' => 0));
        } else {
    ...
    if ($currentgroup and (!$isseparategroups or has_capability('moodle/site:accessallgroups', $context))) {
        ...
        if ($group = groups_get_group($currentgroup)) {
            if (!empty($group->description) or (!empty($group->picture) and empty($group->hidepicture))) {
                ...
                $group->description = file_rewrite_pluginfile_urls($group->description, 'pluginfile.php', $context->id,
    ...
    $personalvars = $DB->get_records('ejsapp_personal_vars', array('ejsappid' => $ejsappid));
    for ($i = 0; $i < count($personalvars); $i++) {
    ...
    $userlist = $DB->get_recordset_sql("$select $from $where $sort", $params, $table->get_page_start(),
        $table->get_page_size());
    ...
    if (!$table->is_downloading($download, $exportfilename)) {
        ...
        $select = new single_select($baseurl, 'ejsappid', $ejsappnames, $ejsappid, null, 'formatmenu');
    ...
    if ($userlist) {
        ...
        foreach ($userlist as $user) {
            ...
            for ($i = 0; $i < count($personalvarsinfo->name); $i++) {
                $data[] = $personalvarsinfo->name[$i];
                $data[] = $personalvarsinfo->value[$i];
            }
    ...
    if (!$table->is_downloading($download, $exportfilename)) {
        ...
        if ($perpage == SHOW_ALL_PAGE_SIZE) {
            ...
            echo $OUTPUT->container(html_writer::link($perpageurl, get_string('showall', '',
                $matchcount)), array(), 'showall');
        }
    }
