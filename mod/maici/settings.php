...
if ($ADMIN->fulltree) {
    ...
    if ($defaultapikey) {
        $defaulassistants = maici_fetch_assistants_array($defaultapikey);
        $select = ['0' => get_string('asistantdefaultselect', 'mod_maici')];
        $defaulassistants = array_merge($select, $defaulassistants);
        $settings->add(new admin_setting_configselect('mod_maici/asistantdefault', get_string('asistantdefault', 'mod_maici'),
                get_string('asistantdefault_desc', 'mod_maici'), 0, $defaulassistants));
    }
}
