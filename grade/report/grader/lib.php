...
class grade_report_grader extends grade_report {
    ...
    public function get_element_header_custom(array $element, bool $withlink = false, bool $icon = true,
        ...
        if ($icon) {
            $header .= grade_helper::get_element_icon($element, $spacerifnone);
        }

        $title = $element['object']->get_name($fulltotal);

        $sectionname = '';
        if ($module = $DB->get_record('modules', ['name' => $element['object']->itemmodule])) {
            $cm = $DB->get_record('course_modules', ['module' => $module->id, 'instance' => $element['object']->iteminstance]);

            $section = $DB->get_record('course_sections', ['id' => $cm->section]);

            $sectionname = course_get_format($section->course)->get_section_name($section->section);
            $sectionname = ' (' . $sectionname . ')';
        }

        $titleunescaped = $element['object']->get_name($fulltotal, false) . $sectionname;
        $header .= $title;
        ...
        if ($withlink && $url =grade_helper::get_activity_link($element)) {
            $a = new stdClass();
            ...
            $headerspan = html_writer::start_tag('span', array('class' => 'gradeitemheader', 'title' => $titleunescaped, 'tabindex' => '0'));
            $headerspan .= $header;
            $headerspan .= html_writer::end_tag('span');
        ...
        foreach ($this->gtree->get_levels() as $row) {
            ...
            foreach ($row as $element) {
                ...
                if ($type == 'filler' || $type == 'fillerfirst' || $type == 'fillerlast') {
                    ...
                    $headerlink = $this->get_element_header_custom($element, true,
