...
class grade_export_form extends moodleform {
    function definition() {
        ...
        // Add checkbox "select with grades".
        $mform->addElement('checkbox', 'selectwithgrades', '', get_string('gradeexportcheckbox', 'local_petel'));

        $PAGE->requires->js_amd_inline("
            require(['jquery'], function($) {
            
                function removeCheckboxes() {
                    $('#id_gradeitemscontainer').find('input').each(function() {                    
                        if (this.checked) {                            
                ...
                $('#id_selectwithgrades').change(function() {
                    if ($(this).is(':checked')) {
                        removeCheckboxes();
                        
                        let elements = $('*').filter(function () {
                            return ($(this).data('hasgrade') === 1);
                        });
        ...
        if ($grade_items = $gseq->items) {
            ...
            foreach ($grade_items as $grade_item) {
                ...
                $sectionname = '';
                if ($module = $DB->get_record('modules', ['name' => $grade_item->itemmodule])) {
                    $cm = $DB->get_record('course_modules', [
                            'course' => $grade_item->courseid,
                            'module' => $module->id,
                            'instance' => $grade_item->iteminstance,
                    ]);

                    $section = $DB->get_record('course_sections', ['id' => $cm->section]);
                    $sectionname = course_get_format($grade_item->courseid)->get_section_name($section->section);
                }

                $course = get_course($grade_item->courseid);
                $groupid = groups_get_course_group($course, true);

                $gui = new graded_users_iterator($course, [$grade_item->id => $grade_item], $groupid);
                ...
                $cmname = !empty($sectionname) ? $grade_item->get_name().' ('.$sectionname.')' : $grade_item->get_name();

                if (!empty($features['idnumberrequired']) and empty($grade_item->idnumber)) {
                    $mform->addElement('checkbox', 'itemids['.$grade_item->id.']', $cmname, get_string('noidnumber', 'grades'), array('data-hasgrade' => $hasgrade));
                    $mform->hardFreeze('itemids['.$grade_item->id.']');
                } else {
                    $mform->addElement('advcheckbox', 'itemids['.$grade_item->id.']', $cmname, null, array('group' => 1, 'data-hasgrade' => $hasgrade));
