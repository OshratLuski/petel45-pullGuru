...
if (!empty($exportplugins)) {
    ...
    $gradeexport = explode(',', get_config('moodle', 'gradeexport'));
    $gradeexport = array_filter($gradeexport);

    if (!empty($gradeexport) && isset($gradeexport[0])) {
        $exportplugin = $gradeexport[0];
    }
